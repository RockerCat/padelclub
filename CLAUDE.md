...
The following are explicitly out of MVP scope — do not implement yet, even opportunistically: club ownership transfer, guest players, global "delete my Mi Pádel Club account" (see Club Membership Principles), and any membership/subscription-tier strategy. Per-club category ranking (see Sport / Ranking Module Principles), single-elimination tournaments per club (see Tournament Module Principles), the sales-assisted commercial funnel — landing → demo request → SUPERADMIN-managed lead → conversion into Entrega de Club (see Funnel Comercial Principles) —, the base commercial layer plus 30-day trial that starts on `claim_club()`, entitlement blocking new-reservation/new-tournament creation while `suspended`, manual Wompi checkout with automatic lifecycle transitions (trial → grace → past_due → suspended, real payments reactivating), and SUPERADMIN observability/configuration of that same lifecycle (see Commercial Subscription Principles) were explicitly requested and are now implemented; global cross-club ranking, ladders, any match/tournament result or point award outside the tournament module, and any commercial mechanism beyond what's listed above (recurring/tokenized billing, refunds, multiple pricing plans, invoicing, any manual SUPERADMIN override of commercial status/payments) remain out of scope.
...
`clubs.is_active` is a separate, platform-level lifecycle control, orthogonal to `archived_at`: only a SUPERADMIN can deactivate or reactivate a club (`platform_deactivate_club`/`platform_reactivate_club`, from the platform panel), never the OWNER, and unlike `archived_at`, reactivation here is already implemented. A deactivated club blocks the same "new commitment" operations archival does, through the same `_require_club_not_archived` guard widened to also reject `is_active = false`. Unlike an archived club (read-only access preserved for members), a deactivated club shows every member — OWNER, ADMIN, PLAYER alike — a full-page `ClubDeactivatedScreen` notice instead of the operational area, with no AppNav and no club switcher rendered around it.

Every automatic or one-click multi-club entry-resolution path — `resolveClubEntryPath`/`resolveActiveMembership` (`src/lib/utils/navigation.ts`, used by login, the auth callback, `/notifications`, `LeaveClubButton`, `/profile`) and `getOwnerClubs` (`src/lib/actions/clubSwitcher.ts`, behind the OWNER "Cambiar de club" modal) — must exclude a `clubs.is_active = false` membership from its eligible/navigable set, exactly like an archived one, never resolving `last_club_id` or a lone remaining membership into a deactivated club when another operationally active membership exists. This was a real, confirmed bug: because `ClubDeactivatedScreen` renders with no nav and no way back to another club, a stale `last_club_id` (or a manual pick from "Cambiar de club") pointing at a platform-deactivated club could trap a multi-club OWNER/ADMIN/PLAYER there even though their other clubs were fine. The one deliberate exception is `/clubs` ("Mis clubes") itself, which still lists a deactivated club on purpose (informational membership overview) — it only had to stop presenting it as an inviting, navigable destination: same card, `Desactivado` badge, reduced emphasis, no `Entrar →`, never a `<Link>`.
...
The 30-day trial does **not** start when the lead arrives, when the demo happens, when the lead is converted, when the pending club is created, or when a claim link is generated — it starts only once the definitive OWNER successfully completes `claim_club()`, since that is the moment the product is actually handed over. See Commercial Subscription Principles below for the trial/subscription layer itself (`commercial_settings`, `club_subscriptions`, `subscription_periods`) — Wompi, real payment collection, entitlement, and any restriction on reservations/tournaments tied to a plan are still explicitly not built.

---

## Commercial Subscription Principles

A club's commercial subscription (`club_subscriptions`) belongs to the **club** (`club_id`, `UNIQUE`), never to a user — commercial history must survive an eventual OWNER change. `payer_profile_id` records who pays today (the OWNER at the moment the trial started) but never defines ownership of the subscription itself. This is deliberately a completely separate concept from the club's operational state: `clubs.is_active`/`archived_at` are never read, written, or reused as a stand-in for commercial status, and a club can be operationally `Activo` with no commercial subscription at all — every pending club before `claim_club()` is exactly that case.

`commercial_settings` is a single global, singleton row (enforced by a unique index on a constant expression, not by application discipline) holding the current base price, promo price/flag, trial length, and currency — today COP $120.000 base, promo enabled at COP $99.990, 30-day trial. Every commercial amount is stored in plain COP units, never Wompi cents. `subscription_periods` records history and, critically, a price **snapshot** per period (`base_price`/`discount_amount`/`final_price`/`currency` captured at creation time) — changing `commercial_settings` later never rewrites a historical period.

Commercial status (`club_subscriptions.status`: `trialing`, `active`, `past_due`, `suspended`, `cancelled`) now reaches `active`/`past_due`/`suspended` through real, automatic paths (Comercial v2 / Fase 3, done — see below); `cancelled` is still never produced by any code path. `suspended` is the value that changes functional behavior — see entitlement below.

The trial starts **exclusively** inside `claim_club()`, immediately after the OWNER transfer is complete (never before — not at lead creation, demo, conversion, pending-club creation, or claim-link generation) via `initialize_club_trial(p_club_id, p_payer_profile_id)`, called in the same function invocation (so the same transaction) as the rest of the claim — a club can never end up claimed without a trial, or vice versa. `initialize_club_trial` is a genuinely internal helper: no `GRANT` to any client role (`anon`/`authenticated` explicitly `REVOKE`d), reachable only from another `SECURITY DEFINER` function's body, and it still independently validates both that `p_payer_profile_id = auth.uid()` (the caller itself, never a third-party profile) and that this profile is a real active OWNER of the target club — two separate checks, neither a substitute for the other. Idempotent by construction: `club_subscriptions.club_id UNIQUE` + `INSERT ... ON CONFLICT (club_id) DO NOTHING` means a retried claim can never create a second subscription, and the function returns before touching `subscription_periods` at all when no row was actually inserted; a partial unique index (`subscription_periods (subscription_id) WHERE status = 'trial'`) is a second, independent guarantee that at most one trial period can ever exist per subscription. Existing clubs from before this phase were deliberately **not** backfilled with a trial — no historical date was inferred, no mass migration ran; only a `claim_club()` completed after this phase shipped starts one.

SUPERADMIN gets read-only observability at `/platform/clubs/[clubId]` ("Estado comercial": status, trial dates + days remaining, current period, next billing, payer, configured base/promo price, últimos períodos y pagos) via `get_platform_club_commercial_status` plus `get_platform_subscription_periods`/`get_platform_subscription_payments`, gated the same way every other platform RPC is. `/platform/subscriptions` (Comercial v2 / Fase 6, done — see below) is the global equivalent: every club, legacy included, with search/filter and a derived phase badge. None of these tables have any RLS policy at all (`ENABLE ROW LEVEL SECURITY` with zero policies, same closed pattern as `commercial_leads`/`club_claim_links`) — every access is mediated by a `SECURITY DEFINER` function.

**Entitlement (Comercial v2 / Fase 2 — done).** `public.get_club_commercial_access(p_club_id)` is the single central source of truth for whether a club's commercial status permits creating new content — returns `commercial_status`, `has_subscription`, `can_create_booking`, `can_create_tournament`, `trial_ends_at`. Rule: no `club_subscriptions` row (legacy club) → allowed; `trialing` → allowed; `active` → allowed; `suspended` → **blocked**; `past_due` (now genuinely reachable via `run_commercial_lifecycle_transitions`, Comercial v2 / Fase 3) → allowed, its own 30-day grace before `suspended`; `cancelled` → allowed, still unreached by any code path and without a decided product behavior. `public._require_commercial_access(p_club_id)` is the internal guard version (same rule, raises `ERRCODE = 'P0008'` instead of returning a boolean) — a genuinely internal helper, no `GRANT` to any client role, called from inside `create_reservation_player`, `create_reservation_admin`, and `create_tournament` immediately after their existing `_require_club_not_archived` call, the only three functions that can ever insert a reservation or a tournament (verified: no other insert path exists anywhere in the codebase). This blocks **only the creation of new reservations and new tournaments** — never edits, never existing/historical/future-already-created reservations or tournaments, never rankings, never results, never any query, never navigation, never SUPERADMIN's own operational access (no bypass was added for it — same rule applies uniformly, including through elevated "Entrar al club" access).

The stable error code `P0008` lets every caller distinguish this from any other rejection without parsing prose. `shared/commercial/access.ts` is the one place that resolves the three per-role messages a caller shows when it sees that code: PLAYER gets a neutral message (title "Reservas temporalmente no disponibles") that never mentions payment/debt/subscription/Wompi; OWNER gets explicit commercial context; ADMIN gets a message pointing to the OWNER, also without any payment mention — verified by an automated test (`scripts/test-commercial-entitlement.ts`) that asserts neither the PLAYER nor the ADMIN message contains any payment-related word. Mobile only needed this on the PLAYER reservation-request path (`mobile/src/lib/reservationMutations.ts`) — OWNER/ADMIN have no operational surface in the app at all (see App Móvil), and mobile never creates tournaments.

OWNER additionally gets a persistent global banner/pill in `src/app/(app)/[club]/layout.tsx` (visibility copy driven by `deriveCommercialPhase`/`getCommercialPillContent`, `shared/commercial/lifecycle.ts`) and a dedicated `/[club]/subscription` screen — the one shell every page under `/[club]/...` renders through, so the banner appears exactly once per screen without having been added page by page. Shown only for the real OWNER (never ADMIN/PLAYER, no SUPERADMIN-specific behavior, elevated "Entrar al club" access included), never for `trialing`/`active`. It never blocks navigation or hides content. This now has a real, functional CTA — "Pagar suscripción"/"Reactivar suscripción" — wired to actual Wompi checkout (Comercial v2 / Fase 3, done — see below), not a placeholder.

**Comercial v2 / Fase 3 (pagos manuales Wompi + ciclo comercial automático — done).** `payments`/`payment_events` (`20261115000007_commercial_payments.sql`, hardened in `20261115000008_wompi_transaction_identity_hardening.sql`) implement single manual Wompi Web Checkout payments — no recurring billing, no tokenization, no Payment Sources. `create_pending_payment` (OWNER-only, never `effective_club_role`/elevated access — moving real money is deliberately never reachable through SUPERADMIN's support bypass) derives price server-side from `commercial_settings`, is idempotent against a double click/double tab via a partial unique index (`payments_one_pending_per_subscription_idx`). `process_wompi_transaction_event` (service_role only, called exclusively from the webhook route after checksum verification) is the single source of truth that flips a subscription to `active` on an `APPROVED` transaction, opening a new `subscription_periods` row that never loses already-paid or already-trialed days (a payment mid-trial or mid-period starts its new period at the existing period's real end, never at "now"). `run_commercial_lifecycle_transitions` (service_role, driven by `/api/cron/commercial-lifecycle`, `CRON_SECRET`-protected, idempotent) is the only thing that ever moves a subscription automatically: `active` past its `current_period_end` → `past_due`; `trialing` past `trial_ends_at` + 14 days grace → `suspended`; `past_due` past `current_period_end` + 30 days grace → `suspended`. These two grace windows (14 days post-trial, 30 days post-past-due) are deliberately not columns — they're pure functions of dates that never change once set (`trial_ends_at`, `current_period_end`), computed identically by SQL and by `deriveCommercialPhase` (TS), never a second, driftable copy. Server-to-server Wompi transaction verification (`src/lib/wompi/verifyTransaction.ts`) is mandatory before ever trusting a webhook event — a confirmed hardening pass closed a real spoofing gap where an altered `reference` on an already-final transaction status could otherwise slip through.

**Still not built — do not add opportunistically:** recurring/tokenized billing (Payment Sources), automatic renewal without a manual checkout, refunds, multiple plans/pricing tiers, invoicing, commercial emails, and any manual SUPERADMIN override of status/payments (see Comercial v2 / Fase 6 below — deliberately observability-only).

**Comercial v2 / Fase 6 (SUPERADMIN observability + configuración global — done).** Pure visibility/configuration on top of the Fase 1-3 lifecycle, never a second lifecycle or a bypass of it: `/platform/subscriptions` (global list, legacy clubs included, search + status filter, reusing the extended `get_platform_clubs_overview` — never a second near-duplicate overview query) and `/platform/settings` (reads/writes the `commercial_settings` singleton: base price, promo flag/price, trial days; currency stays readonly, COP-only). SUPERADMIN can change future pricing only — every write goes through `update_platform_commercial_settings`, which never touches `subscription_periods`/`payments` (their own price snapshot is permanent), never changes a subscription's `status`, never grants entitlement manually. No new SUPERADMIN bypass of entitlement was added anywhere in this phase. Every platform-facing commercial badge (`/platform/clubs`, `/platform/subscriptions`, `/platform/clubs/[clubId]`) is computed via `shared/commercial/platformBadge.ts`, which itself calls `deriveCommercialPhase` — never a second status-labeling rule for the SUPERADMIN audience.
...
This specific Supabase project's Postgres rejects any `UPDATE`/`DELETE` with no `WHERE` clause at all, raising `SQLSTATE 21000` ("UPDATE requires a WHERE clause") — this is not standard PostgreSQL behavior and is easy to forget precisely on a genuine singleton table, where a bare `UPDATE tbl SET ...` (relying on the table having exactly one row, enforced by a unique index on a constant expression rather than a real id) looks structurally correct and was written that way once, only to fail in production with this exact error. `update_platform_commercial_settings` (`commercial_settings`, singleton via `commercial_settings_singleton_idx` on `(true)`, no fixed/known id) is the confirmed real case: fixed by reading `SELECT id INTO v_settings_id FROM commercial_settings LIMIT 1` first (raising `ERRCODE = '55000'` if it's somehow missing, same code `initialize_club_trial` uses for the same condition) and always writing `UPDATE ... WHERE id = v_settings_id` — never `WHERE true`, never a hardcoded id. Any future `UPDATE`/`DELETE` in this codebase, singleton table or not, must carry a real `WHERE`, even when application logic alone would make an unqualified one "safe."
