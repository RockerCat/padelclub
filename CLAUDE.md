...
Commercial status (`club_subscriptions.status`: `trialing`, `active`, `past_due`, `suspended`, `cancelled`) only ever produces `trialing` today — the other four values exist so the schema doesn't need another migration when billing arrives, but nothing in the product currently transitions into them, and none of them currently changes any functional behavior (see below).
...
SUPERADMIN gets read-only observability at `/platform/clubs/[clubId]` ("Estado comercial": status, trial dates + days remaining, current period, next billing, payer, configured base/promo price) via `get_platform_club_commercial_status`, gated the same way every other platform RPC is. No OWNER-facing UI exists yet for a club's own subscription, and none of these tables have any RLS policy at all (`ENABLE ROW LEVEL SECURITY` with zero policies, same closed pattern as `commercial_leads`/`club_claim_links`) — every access is mediated by a `SECURITY DEFINER` function.

**Explicitly not built yet — this is Comercial v2 / Fase 1 only, do not add opportunistically:** Wompi, checkout, real payment collection, webhooks, automatic renewal/billing, grace period, any automatic transition between commercial statuses (e.g. `trialing` → `past_due` on expiry), and — most importantly — **entitlement**: a club's commercial status today has zero effect on any functional behavior. `trialing`/`active`/`past_due`/`suspended`/`cancelled` never block reservations, tournaments, or any other operation, never hide data, and never change navigation for any role. The next planned checkpoint, Comercial v2 / Fase 2 (Entitlement), will block only the **creation** of new reservations and new tournaments based on commercial status — never existing/historical data — and has not been designed or implemented yet.
