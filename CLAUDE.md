...
Ordinary `type='match'` reservations may count toward "partidos jugados" once confirmed and finished, but they never have an official result, winner, wins, losses, points, or ranking impact. Only matches belonging to the tournament module (`tournament_matches`, never `reservations`) can record an official result and affect points — see Tournament Module Principles. A brief, real precedent for why this is stated explicitly: a first attempt (`20260831000001`/`20260831000002`) wired official match results directly onto `reservations`, was applied, and had to be withdrawn (`20260901000001`) once this rule was confirmed — never repeat that coupling.

---

## Tournament Module Principles

A tournament is single-elimination only, scoped to one club, with a fixed bracket of 4, 8, or 16 pairs — never ladders, round robin, group stages, or cross-club play. A tournament is either single-category or combined: `tournaments.category` is always the only category, or the superior one when combined; `tournaments.secondary_category` is `NULL` for a single-category tournament, or the inferior category when combined. Superiority is always enforced by comparing the real `sport_categories.sort_order` of the two codes — never a string comparison, never the digit inside the code, never a numeric sum, and never a parsed commercial name (a combined category may be marketed as "Suma 11" or similar, but that label is never the technical rule).

An entry (`tournament_entries`) represents one registered pair, always exactly two `tournament_entry_members`. A PLAYER can only register a pair that includes themselves, and it is always created `pending` (awaiting OWNER/ADMIN confirmation). OWNER/ADMIN can register any valid pair for any two eligible members, always created `confirmed` directly — the same pending/confirmed split already used by player-created vs. admin-created reservations. Capacity is `pending + confirmed` against `bracket_size`; a `withdrawn` entry frees its slot and is never deleted, only marked. Every registration is validated server-side for category composition, individually per player, never by a global count: a single-category tournament requires both players' real current category (via their `club_member_sport_state`) to equal it exactly; a combined tournament requires both players to belong to `{superior, inferior}` with at most one of the two at the superior category (superior+inferior, inferior+superior, and inferior+inferior are all valid; superior+superior is not). `tournament_entries.category`/`secondary_category` freeze the tournament's modality at the moment the entry is created and are never recalculated afterward, even if the tournament's own configuration changes later.

A bracket is generated exactly once, only via `generate_tournament_bracket`, only when the tournament is `registration_closed` and has exactly `bracket_size` confirmed entries — never regenerated, never edited, never manually reordered. The draw is uniformly random (no seeding or ranking-based placement policy exists). Round topology is expressed only through `tournament_matches.source_match_one_id`/`source_match_two_id` (self-referencing, never `next_match_id`, never a separate `tournament_rounds` table) — round-1 matches materialize both `tournament_entries` directly; every later round is created with its two source matches set and both entries left `NULL` until a future result-recording flow propagates the winner forward.

Any `SECURITY DEFINER` function that returns a full row via `RETURN QUERY SELECT (row).*` must keep its `RETURNS TABLE` column list in exact sync with the real table shape. This is a hard-won rule: adding `tournaments.secondary_category` updated `create_tournament`/`update_tournament`/`register_tournament_entry` (in scope at the time) but left `open_tournament_registration`, `close_tournament_registration`, `cancel_tournament`, and `generate_tournament_bracket` silently broken until a dedicated fix migration caught it — a wildcard row expansion has no static check against its function's declared return shape, so this must be verified by hand every time a column is added to `tournaments` or `tournament_entries`.

A related pitfall, same root cause: when a function's `RETURNS TABLE` declares an output column named `id` (or any other name a table it writes to also has), a bare `RETURNING id` inside that function's own `INSERT` becomes ambiguous between the output column and the real table column — Postgres only catches this at runtime (`42702`), never statically. Any such statement must alias the target table explicitly (`INSERT INTO tbl AS t (...) ... RETURNING t.id`) — `generate_tournament_bracket` hit exactly this in production, fixed the same way (`20260921000001`).

A match can only be scheduled (`schedule_tournament_match`) once both its entries are resolved and the tournament/match status allow it — `isMatchSchedulable` (`tournamentBracket.ts`) only decides whether to show the action client-side, the RPC remains the real gate, never a client-side guess. Recording a result (`record_tournament_match_result`) always derives the winner from the score itself and propagates it to the next round — the frontend never recalculates a winner, only mirrors the same set-validation sequence for UX before submitting. A tournament transitions to `completed` automatically once its final match resolves, never via a separate manual action. Awarding points (`award_tournament_points`) is a distinct, explicit step from completion; `get_tournament_points_summary` is the only read of the resulting ledger rows, classifying `award_state` (`pending`/`ready`/`awarded`/`partial`) purely by comparing the actual movement count against the expected one — never re-deriving points itself.

A completed, single-elimination bracket never produces a real, unambiguous "tercer lugar": it always leaves exactly two tied semifinalists, never a single third-place pair (no consolation match exists in this format). Any UI or export showing tournament results must reflect only champion/runner-up as singular placements — if showing semifinalists at all, label them plainly as semifinalists, never infer or invent a single third place from them.

Once a tournament is `completed` and its points are `awarded`, OWNER/ADMIN get two reuse-only actions from the same detail page: an assisted "Generar noticia" (prefills the existing Noticias creation flow with a draft built purely from the tournament + its real award summary; publishing still goes through the exact same editorial flow, never automatic) and visual PNG exports of the podium/results (see Sports Data Export Principles). Neither creates a second Noticias/Torneos implementation.

---

## Sports Data Export Principles

OWNER/ADMIN can generate shareable PNG images (1080×1350, one shared visual shell — `ShareCardShell`) for the Ranking Top 10 of the currently selected category, and for a completed+awarded tournament's podium and its last two rounds of results. Generation is entirely client-side and on demand only — never at page load, never for a hidden/permanently-mounted card, never uploaded to Storage, never via service role, never published automatically to any external platform; the user always explicitly shares (Web Share API with the image file, never a link-only share) or downloads. A tournament's podium always shows only champion/runner-up (see Tournament Module Principles for why a real third place never exists). Every avatar/logo is resolved to a data URL by the caller before the card ever mounts, each with its own timeout — a failed or slow image always falls back to initials, never blocks the rest of the export.
