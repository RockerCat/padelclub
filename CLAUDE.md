...
Commercial status (`club_subscriptions.status`: `trialing`, `active`, `past_due`, `suspended`, `cancelled`) only ever produces `trialing` today — the other four values exist so the schema doesn't need another migration when billing arrives, and nothing in the product currently transitions a club into any of them automatically (only a manual `UPDATE` for QA can produce `suspended` today). `suspended` is the one value that already changes functional behavior once set — see entitlement below; `active`/`past_due`/`cancelled` still have no functional effect since nothing produces them yet.
...
SUPERADMIN gets read-only observability at `/platform/clubs/[clubId]` ("Estado comercial": status, trial dates + days remaining, current period, next billing, payer, configured base/promo price) via `get_platform_club_commercial_status`, gated the same way every other platform RPC is. None of these tables have any RLS policy at all (`ENABLE ROW LEVEL SECURITY` with zero policies, same closed pattern as `commercial_leads`/`club_claim_links`) — every access is mediated by a `SECURITY DEFINER` function.

**Entitlement (Comercial v2 / Fase 2 — done).** `public.get_club_commercial_access(p_club_id)` is the single central source of truth for whether a club's commercial status permits creating new content — returns `commercial_status`, `has_subscription`, `can_create_booking`, `can_create_tournament`, `trial_ends_at`. Rule: no `club_subscriptions` row (legacy club) → allowed; `trialing` → allowed; `active` → allowed; `suspended` → **blocked**; `past_due`/`cancelled` → allowed for now, deliberately left undecided (no code path produces either value yet — see the migration's own header comment for why this was not invented rather than guessed). `public._require_commercial_access(p_club_id)` is the internal guard version (same rule, raises `ERRCODE = 'P0008'` instead of returning a boolean) — a genuinely internal helper, no `GRANT` to any client role, called from inside `create_reservation_player`, `create_reservation_admin`, and `create_tournament` immediately after their existing `_require_club_not_archived` call, the only three functions that can ever insert a reservation or a tournament (verified: no other insert path exists anywhere in the codebase). This blocks **only the creation of new reservations and new tournaments** — never edits, never existing/historical/future-already-created reservations or tournaments, never rankings, never results, never any query, never navigation, never SUPERADMIN's own operational access (no bypass was added for it — same rule applies uniformly, including through elevated "Entrar al club" access).
...
OWNER additionally gets a persistent global banner ("Tu suscripción no está activa") in `src/app/(app)/[club]/layout.tsx` — the one shell every page under `/[club]/...` renders through, so it appears exactly once per screen without having been added page by page. Shown only when `role === "OWNER"` and `commercial_status === 'suspended'` (never for `trialing`/`active`, never for a legacy club with no subscription, never for ADMIN/PLAYER, no new SUPERADMIN-specific behavior). It never blocks navigation or hides content — same visibility-only philosophy as the pre-existing archived-club banner right above it in that same file, which it deliberately mirrors instead of introducing a new pattern. It currently has **no CTA** — Wompi/reactivation doesn't exist yet. **This is a real, tracked gap, not an oversight:** Comercial v2 / Fase 3 must add a functional "Reactivar suscripción" CTA to this same banner once a real payment destination exists — never before, and never a dead link/placeholder in the meantime.

**Explicitly not built yet — Comercial v2 / Fase 3, do not add opportunistically:** Wompi, checkout, `payments`/`payment_events`, webhooks, Payment Sources/tokenization, automatic charging, renewal, grace period, any automatic transition between commercial statuses (e.g. `trialing` → `past_due` on expiry), billing UI, invoicing, commercial emails, and the "Reactivar suscripción" CTA itself.
