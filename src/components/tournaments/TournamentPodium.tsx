    ...
    <div className="grid grid-cols-3 items-end gap-1.5 sm:gap-2 md:gap-3 lg:gap-4">
      {([1, 2, 3] as const).map((place) => {
        ...
        return placeRows && placeRows.length > 0 ? (
          <PodiumBlock key={place} place={place} rows={placeRows} />
        ) : (
...
function PodiumBlock({ place, rows }: { place: 1 | 2 | 3; rows: TournamentClassificationRow[] }) {
  return (
    <div className={cn("flex flex-col min-w-0", PLACE_ORDER[place])}>
      <div className="flex flex-col items-center gap-1.5 sm:gap-2 lg:gap-3 rounded-t-xl lg:rounded-t-2xl border border-b-0 border-amber-400/25 bg-amber-400/[0.06] px-1.5 py-2 sm:px-2 sm:py-2.5 lg:px-4 lg:py-5 min-w-0">
        ...
        {/* Única zona con scroll — nunca la medalla/título, nunca el
            pedestal. Scrollbar discreto, mismo patrón ya usado en
            ReservationForm.tsx (sin plugin/dependencia nueva). */}
        <div
          className="flex flex-col gap-2 lg:gap-3 w-full divide-y divide-amber-400/10 max-h-[150px] overflow-y-auto min-w-0"
          style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(255,255,255,0.15) transparent" }}
        >
          {rows.map((row, i) => (
            <PodiumEntryRow key={row.entry.id} entry={row.entry} place={place} withDivider={i > 0} />
          ))}
        </div>
      </div>
