...
// transitions) live there, never duplicated inline here (Bloque 2.1 scope).
// `href` lets both the OWNER/ADMIN list (Bloque 2.1) and the PLAYER list
// (Bloque 2.2, /[club]/tournaments) reuse this exact card instead of two
// near-identical copies.
export function TournamentCard({ tournament, href }: { tournament: Tournament; href: string }) {
  return (
    <Link
      href={href}
      className="bg-brand-surface border border-white/10 rounded-2xl p-4 flex flex-col gap-3 hover:border-white/20 transition-colors"
    >
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-brand-muted">
        <span className="inline-flex items-center gap-1">
          ...
          Categoría {tournamentCategoryLabel(tournament.category, tournament.secondary_category)} · {tournament.bracket_size} parejas
        </span>
        <span className="inline-flex items-center gap-1">
