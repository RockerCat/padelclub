...
Explícitamente NO implementado en esta fase: Wompi, checkout, cobro real, webhooks, renovación/cobro automático, grace period, cualquier transición automática entre estados comerciales por vencimiento (p. ej. `trialing` → `past_due`), y — lo más importante — **entitlement**: hoy el estado comercial de un club no afecta ningún comportamiento funcional (no bloquea reservas ni torneos, no oculta datos, no cambia navegación para ningún rol).

Siguiente checkpoint — **Comercial v2 / Fase 2 (Entitlement)**: bloqueará únicamente la **creación** de nuevas reservas y nuevos torneos según el estado comercial, sin afectar históricos ni contenido existente. No implementada ni diseñada todavía.
...
El Funnel Comercial v1 (captación de leads → gestión SUPERADMIN → conversión a Entrega de Club) y Comercial v2 / Fase 1 (capa comercial base + trial automático de 30 días al reclamar el club) tampoco son funcionalidades futuras — ambos están implementados y validados en producción (ver Funnel Comercial (v1) y Comercial v2 — Fase 1). "Pagos" arriba se refiere específicamente a lo que sigue sin implementarse: Wompi, cobro real, renovación automática, y entitlement (bloqueo de creación de reservas/torneos por estado comercial — Comercial v2 / Fase 2) — el estado operativo `Activo` de un club nunca debe confundirse con una suscripción o trial activo.
