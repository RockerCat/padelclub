...
## Próxima fase — explícitamente NO implementado todavía

Wompi, cobros, suscripciones, trial comercial, o cualquier restricción de reservas/torneos por plan. **No confundir el estado operativo `is_active`/`Activo` de un club con una suscripción o trial activo** — son conceptos completamente distintos y no relacionados. El checkpoint decidido para la siguiente fase:

```text
OWNER completa claim_club()
  → inicia el FUTURO trial comercial de 30 días
  → suscripción
  → Wompi
  → entitlement
```

El trial deliberadamente **no** inicia al crear el lead, hacer la demo, convertirlo, crear el pending club, ni generar el link de reclamo — solo al `claim_club()` exitoso, porque ese es el momento real de entrega del producto al cliente. No existe todavía ninguna columna/tabla de trial o suscripción en el esquema.
...
El Funnel Comercial v1 (captación de leads → gestión SUPERADMIN → conversión a Entrega de Club) tampoco es una funcionalidad futura — está implementado y validado en producción (ver Funnel Comercial (v1)). "Pagos" arriba se refiere específicamente a Wompi/cobros/suscripciones/trial comercial/entitlement, que siguen sin implementarse — el estado operativo `Activo` de un club nunca debe confundirse con una suscripción o trial activo.
