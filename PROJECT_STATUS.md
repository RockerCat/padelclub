...
* Última actualización: 11 de septiembre de 2026 (Comercial v2 / Fase 6 — Superadmin Comercial: implementada y corregida, todavía sin commit/push. Ver "Comercial v2 — Fase 6 (Superadmin Comercial)" para el detalle completo; resumen: `/platform/subscriptions` (nueva — listado global de todos los clubes con estado comercial efectivo, legacy incluidos, búsqueda + filtro por fase, métricas simples) y `/platform/settings` (nueva — lectura/edición de `commercial_settings`: precio base, promo, trial days; moneda readonly) activadas en el sidebar de Platform Admin, dejando de ser "Próx."; `/platform/clubs` gana columna "Suscripción" separada del "Estado" operativo; `/platform/clubs/[clubId]` gana historial de "Períodos" y "Pagos". Una sola migración nueva (`20261115000009_platform_subscriptions_settings.sql`) extiende `get_platform_clubs_overview` (reutilizada por ambas pantallas, nunca duplicada) y agrega 4 RPCs de solo-lectura/escritura-acotada, todas gateadas por `is_platform_admin`; ningún bypass del lifecycle comercial, ninguna edición manual de estado/trial/períodos/pagos. Badge de estado unificado vía `shared/commercial/platformBadge.ts` (nuevo, envuelve `deriveCommercialPhase`). Además, tres bugs reales de navegación multi-club encontrados durante el cierre de QA de esta fase y corregidos — ver "Corrección — Navegación Multi-Club y Club Desactivado": un OWNER/ADMIN/PLAYER con varios clubes podía quedar atrapado en un club operativamente desactivado (sin AppNav ni selector) en vez de entrar a sus otros clubes activos, tanto por `last_club_id` (`resolveClubEntryPath`/`resolveActiveMembership`, `src/lib/utils/navigation.ts`) como por selección manual (`getOwnerClubs`, modal "Cambiar de club") — ambos ahora excluyen `clubs.is_active = false` del conjunto elegible, igual que ya excluían `archived_at`; `/clubs` ("Mis clubes") sigue mostrando el club desactivado a propósito, pero ya con badge "Desactivado", atenuado y sin `Entrar →`. Un cuarto bug real, específico de Fase 6, encontrado y corregido durante QA — ver "Corrección — `update_platform_commercial_settings` (UPDATE sin WHERE)": guardar `/platform/settings` fallaba siempre (`SQLSTATE 21000`, "UPDATE requires a WHERE clause" — este proyecto de Supabase rechaza cualquier `UPDATE`/`DELETE` sin `WHERE`, incluso sobre una tabla singleton) — corregido resolviendo el id real de la fila singleton en runtime y escribiendo `WHERE id = v_settings_id`, nunca `WHERE true` ni un UUID hardcodeado; verificado en producción por el usuario (`99990 → 99991` persiste tras recargar, restaurado a `99990`). De paso, documentada retroactivamente **Comercial v2 / Fase 3 (Pagos Wompi + Ciclo Automático)** — implementada en una sesión anterior sin haber sido registrada aquí: pagos manuales Wompi (`payments`/`payment_events`, `create_pending_payment`, `process_wompi_transaction_event`), transiciones automáticas de estado por cron (`run_commercial_lifecycle_transitions` — `active`→`past_due`, `trialing`/`past_due`→`suspended` con gracia de 14/30 días), hardening de verificación server-to-server contra spoofing de transacciones, y el CTA "Reactivar suscripción" ya funcional en el banner OWNER — ver esa sección para el detalle reconstruido a partir del SQL real. `npx tsc --noEmit`, `npm run build`, `test:commercial-entitlement` (30/30) y `test:commercial-payments` (126/126) limpios en cada checkpoint; ESLint global sin regresión sobre el baseline (73 errores/44 warnings). Ver CLAUDE.md → Commercial Subscription Principles, Club Archival Principles y Shared View & Data Patterns.)
* Actualización anterior: 10 de septiembre de 2026 (Comercial v2 / Fase 2 — Entitlement: implementada, migrada y validada en producción. Ver "Comercial v2 — Fase 2 (Entitlement)" para el detalle completo; resumen: `get_club_commercial_access`/`_require_commercial_access` como fuente central de verdad (sin suscripción/`trialing`/`active` → permite; `suspended` → bloquea con `ERRCODE = 'P0008'`; `past_due`/`cancelled` quedan permitidos, sin decisión de producto todavía — ningún camino de código los produce hoy). Enforcement server-side dentro de las 3 únicas funciones que pueden crear el recurso real (`create_reservation_player`, `create_reservation_admin`, `create_tournament` — verificado que no existe ningún INSERT directo alternativo en todo el repo), nunca solo en el frontend. Mensajes por rol centralizados en `shared/commercial/access.ts` (PLAYER neutral, OWNER con contexto comercial, ADMIN remite al propietario — ninguno de los dos últimos menciona pago, verificado con test automatizado). Nuevo banner global persistente para OWNER ("Tu suscripción no está activa") en `src/app/(app)/[club]/layout.tsx` — el shell que envuelve toda página del club, visible una sola vez por pantalla, solo cuando `suspended`, sin CTA todavía (Wompi no existe). La suspensión nunca afecta reservas/torneos existentes, históricos o ya creados, ni rankings, resultados o cualquier consulta — bloquea únicamente la creación de contenido nuevo. Mobile: solo se tocó el flujo PLAYER de creación de reservas. Migración aplicada: `20261115000006_commercial_entitlement.sql`. QA real en producción con `Club Demo Comercial` cubrió `trialing`/`suspended` (PLAYER/OWNER/ADMIN)/legacy/restauración — ver detalle en esa sección. **Explícitamente pendiente — siguiente checkpoint, Comercial v2 / Fase 3 (Pagos/Wompi):** Wompi, checkout, cobro automático, renovación, grace period, transiciones automáticas de estado, billing UI, facturación, y el CTA funcional "Reactivar suscripción" que debe agregarse al mismo banner OWNER ya existente. Ver CLAUDE.md → Commercial Subscription Principles.)
* Actualización anterior: 9 de septiembre de 2026 (Comercial v2 / Fase 1 — Capa Comercial Base + Trial de 30 días: implementada, migrada y validada en producción. Ver "Comercial v2 — Fase 1 (Capa Comercial Base + Trial)" para el detalle completo; resumen: nuevas `commercial_settings` (configuración global única: base COP $120.000, promo COP $99.990, trial 30 días), `club_subscriptions` (una fila por club, `club_id UNIQUE` — la suscripción pertenece al CLUB, nunca al usuario/OWNER) y `subscription_periods` (historia con snapshot de precio propio por período). El estado comercial (`trialing`/`active`/`past_due`/`suspended`/`cancelled`, hoy solo se produce `trialing`) es un concepto completamente separado del estado operativo `is_active`/`Activo` del club — nunca se sobrecarga esa columna. El trial inicia exclusivamente dentro de `claim_club()` exitoso, vía `initialize_club_trial` (helper interno sin GRANT a ningún rol de cliente, idempotente por `club_id UNIQUE` + `ON CONFLICT DO NOTHING` + índice único parcial para el período trial), ejecutado en la misma transacción que el resto del claim — nunca antes (no al crear el lead, hacer la demo, convertir, crear el pending club, ni generar el link de reclamo). Sin backfill retroactivo para clubes existentes. Tres migraciones aplicadas: `20261115000003_commercial_base_schema.sql`, `20261115000004_claim_club_initializes_trial.sql`, `20261115000005_platform_club_commercial_status.sql`. QA real en producción con `Club Demo Comercial`/`Alex Test Commercial`: pre-claim confirmado `Sin suscripción comercial inicializada` (0 filas en ambas tablas); post-claim confirmado exactamente 1 fila `trialing` (trial 9 sep → 9 oct 2026, payer/período/próximo cobro correctos) y exactamente 1 período `trial` en COP $0, con SUPERADMIN mostrando todo correctamente en `/platform/clubs/[clubId]`. **Explícitamente pendiente — siguiente checkpoint, Comercial v2 / Fase 2 (Entitlement):** Wompi, cobro real, renovación automática, grace period, transiciones automáticas por vencimiento, y bloqueo de creación de nuevas reservas/torneos por estado comercial (nunca sobre históricos) — nada de esto existe todavía; el estado comercial hoy es puramente informativo. Ver CLAUDE.md → Commercial Subscription Principles.)
* Actualización anterior: 9 de septiembre de 2026 (Funnel Comercial v1: implementado, migrado y validado en producción de punta a punta. Ver "Funnel Comercial (v1)" para el detalle completo; resumen: landing reorientada a OWNER, CTA principal "Solicitar una demo" → `/demo` (página pública compartible, sin login) — self-service (`RegisterMenu`, `/auth/signup`, `/clubs/create`) intacto, ya no es el CTA principal. Nueva tabla `commercial_leads` (RLS completamente cerrada, sin policies — mismo patrón que `club_claim_links`) con una sola RPC de escritura pública (`create_commercial_lead`, sin parámetro `status`/`source`/`notes`/`converted_club_id` en su firma — estructuralmente imposible que el cliente se autodeclare `won`). Gestión SUPERADMIN nueva en `/platform/leads` (listado/búsqueda/filtro) y `/platform/leads/[leadId]` (estado, `demo_at`, motivo de pérdida, notas, contacto WhatsApp). Estados: `new` → `contacted` → `demo_scheduled` → `demo_completed` → `won`/`lost`. Conversión (`platform_convert_lead_to_pending_club`) reutiliza `platform_create_pending_club` sin reimplementarlo, dentro de un mismo bloqueo `FOR UPDATE` — deja `converted_club_id`/`converted_at`/`status='won'` atómicamente; solo permitida desde `demo_completed`, verificado tanto en UI (botón "Crear club" oculto antes de ese estado, incluida la ruta `?lead=`) como server-side (la RPC rechaza cualquier estado anterior). Dos migraciones aplicadas en producción: `20261115000001_commercial_leads_funnel.sql` (tabla + RPCs) y `20261115000002_lead_conversion_requires_demo_completed.sql` (agrega el gate de `demo_completed` a la conversión). Bug real corregido en el camino: el contacto por WhatsApp de un lead duplicaba el mensaje prellenado — causa raíz era el endpoint (`api.whatsapp.com/send` con `phone`+`text` combinados), no la construcción del string; corregido a `wa.me/<phone>?text=`, ver CLAUDE.md → WhatsApp Share Principles. **Explícitamente pendiente para la siguiente fase, no confundir con el estado operativo `Activo` de un club:** trial comercial de 30 días, suscripciones, Wompi, entitlement — el trial no inicia al crear el lead, hacer la demo, convertir el lead, crear el pending club ni generar el link de reclamo; inicia únicamente cuando el OWNER definitivo completa `claim_club()` con éxito. Ver CLAUDE.md → Funnel Comercial Principles.)
* Actualización anterior: 27 de agosto de 2026 (Cierre de MVP mobile para tiendas: alcance v1 de OWNER/ADMIN, rediseño de Login, un bug crítico real de login/Dashboard corregido, y presentación compacta de Torneos en mobile y WEB. **Mobile — alcance v1 de OWNER/ADMIN:** ya no hay ningún dashboard/tab operativo nativo para OWNER/ADMIN — una cuenta con ese rol en su club activo ve una única pantalla estructural (`OwnerAdminWebScreen`, decidida en `RootNavigator.tsx` antes de montar `AppTabs`, nunca un redirect) con un botón a `https://mipadel.club/auth/login` y "Cerrar sesión", sin "Cambiar de club" (deliberado); depende del rol en el CLUB ACTIVO, nunca de otra membresía OWNER/ADMIN que la cuenta tenga en otro club — PLAYER queda completamente intacto. **Mobile — Login rediseñado:** isotipo real (`assets/branding/logo-icon.png`, mismo PNG que ya usa WEB, copiado a `mobile/assets/` porque Metro no resuelve fuera de su `projectRoot`/`shared/`) + wordmark compuesto por texto (mismo patrón que `BrandLogo.tsx` en WEB, nunca un logo recreado desde cero), titular "Todo tu pádel en un solo lugar", fondo real (`assets/background.png` vía `ImageBackground` + overlay muy sutil), "Conocer más →" hacia `https://mipadel.club/` y "Crear cuenta" (mismo destino, ya que el registro real requiere elegir jugador/propietario, algo que hoy solo el landing resuelve) — lógica de autenticación intacta. **Bug crítico real corregido — login/Dashboard:** un PLAYER con membership activa real caía en la pantalla de "sin club" en vez de su Dashboard, y el Dashboard mostraba el email en vez de `profiles.full_name`/`avatar_url` — causa raíz: `resolveActiveMembership`/`getIdentity` descartaban el `error` de su propia consulta a Supabase, así que un fallo transitorio de esa única consulta (carrera real justo después de `signIn()`) se leía exactamente igual que "el usuario no tiene nada" en vez de propagarse; ahora ambas verifican `error` y reintentan una vez antes de rendirse. Ver CLAUDE.md → Shared View & Data Patterns para la regla permanente. **Mobile — Torneos:** el tab inferior podía reabrir directo el detalle de un torneo visitado antes (el stack anidado de ese tab no se resetea solo al re-tocar el ícono) — corregido con un listener `tabPress` que vuelve al listado, sin tocar push/deep links; "Mis torneos" del Dashboard y el listado principal de la tab ahora usan el mismo patrón compacto (`CompactCarouselCard`) que ya usaba Página del club — carrusel horizontal en el Dashboard, grid de 2 columnas en el listado dedicado. **WEB — misma presentación compacta:** `TournamentsGrid` (listado de `/[club]/tournaments` y `/[club]/admin/tournaments`) pasa de 1 a 2 columnas en viewport mobile (desktop/tablet sin cambios); "Mis torneos" del Dashboard WEB gana el mismo carrusel horizontal compacto (`CompactCarouselCard`, WEB) ya usado en Página del club, visible solo en mobile, con el grid completo intacto desde `sm:`. Ver CLAUDE.md → Shared View & Data Patterns.)
* Actualización anterior: 20 de agosto de 2026 (Push notifications Fase 1 en mobile, flujo público de clubes con reingreso confiable, y saneamiento completo del build de producción de WEB. Ver "Push Notifications (Mobile, Fase 1)" y "Corrección de Build — Tipos Generados de Supabase vs. SQL Real" para el detalle completo; resumen: **Mobile — Push:** registro de token (`register_push_token` RPC) + tap-to-open real, con Edge Function `send-push` + trigger de envío automático en cada INSERT de `notifications`. **Mobile — flujo público de clubes:** nueva `PublicClubScreen` (paridad funcional con `/[club]`/`RequestAccessButton` de WEB — CTA "Solicitar ingreso"/"Unirme"/"Solicitud pendiente"/"Entrar al club", revalidado contra Supabase en cada foco), `ChangeClubScreen` ya no ejecuta join/request directo al tocar una fila, `WhatsAppRequiredModal` extraído sin duplicar. **Bugs reales de sincronización de acceso corregidos, en cadena:** reingreso bloqueado tras desactivación y tras rechazo histórico (`create_join_request`/`approve_join_request`/`join_public_club` ahora reactivan una membership existente y ya no tratan un `club_join_requests.status` histórico como ban permanente — dos migraciones nuevas); loop infinito de splash al revalidar sin club activo (`ClubContext` gana `reloading`, separado de `loading`, que sigue siendo bootstrap-only); carrera de navegación que dejaba al usuario viendo "Solicitud pendiente" tras una aprobación real (`awaitingClubEntry`, espera determinista a que el render se asiente); una push ya procesada reapareciendo en cada `AppState→active` o tras un reload de Metro (`Notifications.clearLastNotificationResponse()`, nunca solo un dedup en memoria). Ver CLAUDE.md → Notifications & Live-Update Principles. **WEB — Build de producción:** `npm run build`/`tsc` quedaron bloqueados por una cadena de mismatches `string | null` vs. el tipo generado de Supabase para RPCs sin `DEFAULT` (Torneos, Noticias, 3 RPCs de Reservas, Tarifas, Claim Club, retorno de `get_club_claim_status`) — cada uno investigado contra el SQL real antes de corregir, nunca un `?? undefined`/`as string` a ciegas; más `supabase/functions/` (Edge Functions, runtime Deno) excluido de `tsconfig.json`, que las barría sin querer al typecheck de Next.js. Ver CLAUDE.md → Shared View & Data Patterns.)
* Actualización anterior: 13 de agosto de 2026 (Ronda extensa de paridad PLAYER en la app móvil, más un fix real de Torneos aplicable a ambas plataformas. **Torneos — estado temporal real:** `registration_open` podía quedar visualmente abierto para PLAYER (sin cron/scheduler, `status` nunca se auto-transiciona) aunque `registration_closes_at`/`starts_at` ya hubieran pasado — nuevas `effectivePlayerTournamentStatus`/`isRegistrationTemporallyOpen`/`hasTournamentStarted` (`shared/tournaments/actions.ts`) son ahora la única fuente para lo que ve un PLAYER en badges/"Próxima actividad"/"Mis torneos"/CTA "Inscribirme", en WEB y mobile por igual; `register_tournament_entry` gana el mismo chequeo server-side (migración nueva, pendiente de aplicar manualmente — ver Módulo de Torneos). **Mobile — navegación PLAYER:** el tab bar de PLAYER dejó de compartir el navegador de OWNER/ADMIN sin ningún guard de rol — un gap real de privacidad, no solo cosmético (ver más abajo) — y ahora es Dashboard | *(nombre real del club, ícono `tennis-ball`)* | Reservaciones | Ranking | *(Torneos, ícono `tennis-racket`)*, igual orden que `AppNav.tsx` en WEB; ambos íconos vía `@lucide/lab` (dependencia nueva, confirmada compatible con Expo Go antes de instalarse). **Mobile — Página del club:** de placeholder a paridad real con `/[club]/home` — mapa interactivo de ubicación (`react-native-maps`, dependencia nueva, "Included in Expo Go" sin config adicional), hero completo (portada, logo superpuesto, nombre, ciudad, indicador público/privado, descripción), detalle real de noticia, y nuevas secciones "Torneos activos"/"Torneos finalizados" (bucketing compartido, `partitionPlayerTournaments`) — todo como carruseles horizontales compactos (Noticias/Torneos, ~44% de card, imagen+título) tanto en mobile como en el breakpoint mobile de WEB, con Galería reubicada entre Noticias y Torneos y ahora tocable en mobile (reutiliza `ImagePreviewModal`, ya existente para portadas de torneo). **Mobile — Ranking:** de placeholder a paridad completa con `RankingView.tsx` — selector de categoría, tarjeta "Tu posición", podio Top 3 (alineado visualmente con `TournamentPodium`, incluida corrección de centrado del avatar dentro de cada pedestal), listado completo, y "Compartir Ranking" (mismo mensaje de WhatsApp que WEB, sin la generación de imagen PNG — `html-to-image` no tiene equivalente en React Native). Extracción nueva a `shared/`: `shared/clubs/publicPageData.ts`, `shared/players/ranking.ts` (`computeRankingPresentation`/`buildRankingWhatsappMessage`, ahora también consumidas por `RankingView.tsx`/`RankingExportButton.tsx` en WEB — ninguna lógica de podio/empates duplicada entre plataformas). OWNER/ADMIN sin ningún cambio de navegación/UX en todo lo anterior. Ver CLAUDE.md → Tournament Module Principles, Sports Data Export Principles, Shared View & Data Patterns.)
...
* `TournamentsGrid` (listado, compartido por `/[club]/tournaments` y `/[club]/admin/tournaments`): en viewport mobile (`<640px`) el grid de resultados pasó de 1 a 2 columnas (`grid-cols-2 lg:grid-cols-3`, antes `grid-cols-1 sm:grid-cols-2 lg:grid-cols-3`) — desktop/tablet (`≥640px`) sin ningún cambio visual; mismo `TournamentCard`, mismos filtros, misma navegación al detalle. Mismo ajuste espejo del lado mobile app — ver "App Móvil" → "Qué está portado, por módulo" → Torneos
...
5. **Mis torneos** — tarjetas con la clasificación oficial real (`computeTournamentClassification`, respeta empates), compañero, fecha y puntos; grid completo desde `sm:` sin cambios, y desde esta ronda un carrusel horizontal compacto (`CompactCarouselCard`, mismo componente/patrón que Página del club) en viewport mobile — mismos datos/filtro/navegación al detalle
...
Estado: ✅ DONE — implementado, migrado y validado end-to-end en producción (9 de septiembre de 2026). Ver CLAUDE.md → Funnel Comercial Principles para las reglas permanentes; este apartado documenta el detalle de lo construido. Reutiliza Entrega de Club (ver Panel de Plataforma (SUPERADMIN) más arriba) sin modificar su mecánica — ningún segundo modelo de OWNER temporal, invitación, creación de club, transferencia u onboarding.

## Landing y captación (`/demo`)

* CTA principal del Hero cambiado de "Registrarme" a "Solicitar una demo" → `/demo`; copy reorientado a dueños/administradores de club. CTA secundario "Explorar clubes →" y todo el self-service existente (`RegisterMenu`, `/auth/signup`, `/clubs/create`) quedan intactos, ya no son el CTA principal
* `/demo` es una página pública normal (dentro de `(marketing)`, hereda Navbar/Footer), nunca un modal — pensada para compartirse directamente por Instagram/Facebook/WhatsApp/email/campañas. Formulario: nombre, club, ciudad, WhatsApp (validado con `shared/utils/phone.ts`, mismo normalizador de todo el producto), número de canchas (opcional), y checkbox de consentimiento obligatorio con link a `/privacy` (`privacy_consent_at` se persiste en el lead)
* Anti-spam de bajo costo: honeypot oculto (un valor no vacío ahí responde éxito sin escribir fila, para no darle señal a un bot) más los `CHECK` de la tabla — sin CAPTCHA, sin rate limiting distribuido
* Todos los campos del formulario son controlados (`value`/`onChange`) — un envío fallido por validación (p. ej. sin consentimiento) nunca borra lo ya escrito, corrigiendo un comportamiento real de React 19 (`<form action={...}>` resetea inputs no controlados tras cada submit, incluido uno fallido)

## `commercial_leads` y gestión SUPERADMIN

* Tabla nueva, RLS completamente cerrada (`ENABLE ROW LEVEL SECURITY` + `REVOKE ALL FROM PUBLIC, anon, authenticated`, cero policies) — mismo patrón que `club_claim_links`/`club_claim_events`, elegido deliberadamente para que la clase de bug ya vivida en este proyecto (una policy SUPERADMIN sin `TO authenticated` evaluándose también para `anon`, ver incidente arriba) sea estructuralmente imposible aquí: no hay ninguna policy que pudiera tener ese defecto
* `create_commercial_lead` (`SECURITY DEFINER`, `anon`+`authenticated`) es la única superficie de escritura pública — su firma no acepta `status`/`source`/`notes`/`converted_club_id` como parámetro en absoluto, así que ningún caller puede autodeclararse `won`/`lost`/convertido ni inyectar notas internas; `status='new'`, `source='website'` y `converted_club_id=NULL` quedan hardcodeados en el `INSERT`
* Lectura/edición vía `get_platform_leads_overview`, `get_platform_lead_detail`, `update_commercial_lead_status`, `update_commercial_lead_notes` (todas `SECURITY DEFINER`, `authenticated`-only, gateadas con el mismo `EXISTS (... profiles.is_platform_admin ...)` que el resto del panel de plataforma)
* `/platform/leads` (listado, búsqueda por nombre/club/ciudad/WhatsApp, filtro por estado vía `FilterDropdown`) y `/platform/leads/[leadId]` (cambio de estado, `demo_at`, motivo de pérdida, notas internas, botón "Contactar por WhatsApp") — nuevo ítem "Prospectos" en `PlatformNav.tsx`, entre Clubes y Usuarios
* Estados deliberadamente simples, no un CRM: `new` → `contacted` → `demo_scheduled` → `demo_completed` → `won`/`lost`. `demo_at`/`lost_reason` nunca se borran automáticamente al cambiar de estado — solo se sobrescriben cuando se manda un valor nuevo no vacío, preservando historia
* Datos del lead (nombre, club, ciudad, WhatsApp, notas) nunca aparecen en ninguna superficie OWNER/ADMIN/PLAYER, página pública, ni Mobile

## Conversión — Lead → Pending Club (Entrega de Club)

* `platform_convert_lead_to_pending_club(p_lead_id, p_name, p_slug, p_visibility)` — bloquea el lead (`FOR UPDATE`), llama a `platform_create_pending_club` tal cual (nunca reimplementa su lógica) y, en la misma transacción, fija `converted_club_id`/`converted_at`/`status='won'` — atómico: nunca puede quedar un club creado con su lead sin convertir, o viceversa
* Solo permitida desde `status='demo_completed'` — verificado dos veces: la UI oculta "Crear club" en cualquier estado anterior (mostrando en su lugar "Disponible cuando el prospecto llegue a 'Demo realizada'"), y la ruta `/platform/clubs/create?lead=<id>` muestra un mensaje explicativo en vez del formulario para un lead no elegible; server-side, la RPC rechaza cualquier estado distinto de `demo_completed` con `not_demo_completed`. `won` nunca es un prerrequisito — la conversión exitosa es precisamente lo que produce `won`
* Un lead ya convertido se rechaza idempotentemente (`already_converted`) en vez de reprocesarse o generar un segundo club; `/platform/clubs/create?lead=<id>` redirige directo al club existente si el lead ya está convertido
* `/platform/clubs/create` (`PendingClubFields.tsx`) acepta `?lead=<leadId>`: prellena el nombre del club (y el slug sugerido) desde el lead, sin duplicar el formulario existente — la pantalla y el flujo de Entrega de Club (configurar, generar link, claim) siguen siendo exactamente los mismos que sin lead
* Copy ajustado en esa misma pantalla: "El club se crea de inmediato..." (antes "queda activo de inmediato") — para no confundir el estado operativo con una futura activación comercial/trial

## Migraciones

* `20261115000001_commercial_leads_funnel.sql` — tabla `commercial_leads` + las 6 RPCs (`create_commercial_lead`, `get_platform_leads_overview`, `get_platform_lead_detail`, `update_commercial_lead_status`, `update_commercial_lead_notes`, `platform_convert_lead_to_pending_club`). Hardening: `SECURITY DEFINER`, `SET search_path = ''` (vacío, no `public`) con toda tabla/función propia calificada explícitamente con `public.` — endurecimiento deliberado por encima del patrón `search_path = public` usado en el resto del repo, verificado columna por columna antes de aplicarse
* `20261115000002_lead_conversion_requires_demo_completed.sql` — reemplaza únicamente `platform_convert_lead_to_pending_club` para agregar el gate de `demo_completed` descrito arriba; ninguna otra función tocada
* Ambas confirmadas corriendo en producción

## Bug real corregido — WhatsApp duplicado

Contactar un lead por WhatsApp duplicaba el mensaje prellenado en el compositor del destinatario. Causa raíz: no la construcción del string (un solo mensaje, un solo `encodeURIComponent`), sino el endpoint — `https://api.whatsapp.com/send?phone=<num>&text=<msg>` (combinar `phone`+`text` en ese endpoint específico) duplica el texto en su cadena de redirección interna; ningún otro lugar del repo combinaba esos dos parámetros ahí. Corregido a `https://wa.me/<phone>?text=<encoded>` — el mismo formato que ya usa `MARKETING_WA_URL` sin problema, distinto del caso realmente prohibido (`wa.me/?text=` sin teléfono, que sí corrompe UTF-8). Ver CLAUDE.md → WhatsApp Share Principles.

## Próxima fase — actualizado

El checkpoint que este apartado dejaba abierto (`claim_club()` → trial comercial de 30 días → suscripción) **ya está implementado** — ver "Comercial v2 — Fase 1 (Capa Comercial Base + Trial)" más abajo para el detalle completo. Wompi, cobros reales, renovación automática y entitlement siguen sin implementarse (ver esa misma sección → "Pendiente / próxima fase").

---

# Comercial v2 — Fase 1 (Capa Comercial Base + Trial)

Estado: ✅ DONE — implementada, migrada y validada en producción (9 de septiembre de 2026). Ver CLAUDE.md → Commercial Subscription Principles para las reglas permanentes; este apartado documenta el detalle de lo construido y el QA real ejecutado.

## Modelo

* `commercial_settings` — configuración comercial centralizada, una sola fila global (índice único sobre una expresión constante, no solo disciplina de aplicación). Valores actuales: precio base **COP $120.000**, promo activa a **COP $99.990**, trial de **30 días**, moneda **COP**. Montos en unidades COP normales, nunca centavos Wompi
* `club_subscriptions` — una fila comercial por club (`club_id UNIQUE`). **La suscripción pertenece al CLUB, no al usuario** — `payer_profile_id` registra quién paga hoy (el OWNER al momento de iniciar el trial), nunca de quién "es" la suscripción; el historial comercial sobrevive a un futuro cambio de OWNER. Estados soportados por el esquema: `trialing`, `active`, `past_due`, `suspended`, `cancelled` — hoy solo se produce realmente `trialing`
* `subscription_periods` — historia de períodos con **snapshot de precio** propio por período (`base_price`/`discount_amount`/`final_price`/`currency` capturados al crearse); un cambio futuro en `commercial_settings` nunca reescribe un período ya creado
* El estado comercial es un concepto completamente separado del estado operativo del club — `clubs.is_active`/`archived_at` nunca se leen, escriben ni reutilizan como sustituto de `club_subscriptions.status`. Un club puede estar operacionalmente `Activo` sin ninguna suscripción comercial (todo pending club antes de `claim_club()`)
* Las tres tablas están RLS-cerradas (sin ninguna policy) — todo acceso pasa por funciones `SECURITY DEFINER`

## Inicio del trial — únicamente dentro de `claim_club()`

* `initialize_club_trial(p_club_id, p_payer_profile_id)` corre dentro de la misma invocación de `claim_club()` (misma transacción), inmediatamente después de que la transferencia de OWNER ya está completa — nunca antes: no al crear el lead, hacer la demo, convertirlo, crear el pending club, ni generar el link de reclamo
* Helper genuinamente interno: sin `GRANT` a `anon`/`authenticated`, solo alcanzable desde el cuerpo de `claim_club`. Valida internamente que `p_payer_profile_id = auth.uid()` (el caller mismo) y que ese perfil sea OWNER activo real del club — dos checks independientes
* Idempotente por construcción: `club_subscriptions.club_id UNIQUE` + `INSERT ... ON CONFLICT (club_id) DO NOTHING`; un índice único parcial garantiza además a lo sumo un período `trial` por suscripción
* Ningún club existente antes de esta fase recibió trial retroactivo — sin backfill masivo, sin fecha histórica inferida; solo un `claim_club()` completado después de esta fase inicia uno
* Observabilidad SUPERADMIN: sección "Estado comercial" en `/platform/clubs/[clubId]` (`get_platform_club_commercial_status`) — estado, fechas de trial + días restantes, período actual, próximo cobro, payer, precio base/promo configurados; `Sin suscripción comercial inicializada` cuando el club todavía no tiene fila

## Migraciones aplicadas en producción

* `20261115000003_commercial_base_schema.sql` — las 3 tablas + `initialize_club_trial`
* `20261115000004_claim_club_initializes_trial.sql` — integra `initialize_club_trial` dentro de `claim_club()` (una sola línea nueva sobre la función existente, sin reimplementarla)
* `20261115000005_platform_club_commercial_status.sql` — RPC de lectura SUPERADMIN

## QA real en producción — `Club Demo Comercial`

**Antes del claim:** club operacionalmente `Activo`; SUPERADMIN mostraba `Sin suscripción comercial inicializada`; confirmado en base de datos 0 filas en `club_subscriptions` y 0 en `subscription_periods` para ese club.

**Después de aceptar la entrega con `Alex Test Commercial`:** OWNER transferido correctamente, entrega marcada `Reclamado` (comportamiento de Entrega de Club sin cambios); `club_subscriptions` con exactamente 1 fila `trialing`, trial 9 sep → 9 oct 2026 (30 días), payer correcto, `current_period`/`next_billing_at` correctos; `subscription_periods` con exactamente 1 período `trial` en COP $0; SUPERADMIN mostró correctamente estado "En prueba", fechas, payer y precios configurados.

**Conclusión validada:**

```text
Pending Club sin suscripción
  → claim_club()
  → OWNER definitivo
  → trialing
  → trial de 30 días
...
El checkpoint que este apartado dejaba abierto (entitlement: bloquear la creación de reservas/torneos por estado comercial) **ya está implementado** — ver "Comercial v2 — Fase 2 (Entitlement)" más abajo. Wompi, cobro real, renovación automática, grace period y transiciones automáticas de estado siguen sin implementarse (ver esa misma sección → "Pendiente / próxima fase").

---

# Comercial v2 — Fase 2 (Entitlement)

Estado: ✅ DONE — implementada, migrada y validada en producción (10 de septiembre de 2026). Ver CLAUDE.md → Commercial Subscription Principles para las reglas permanentes; este apartado documenta el detalle de lo construido y el QA real ejecutado.

## Fuente central de verdad

* `public.get_club_commercial_access(p_club_id)` — RPC de lectura, accesible a cualquier miembro real del club (PLAYER incluido) o SUPERADMIN con acceso elevado. Devuelve `commercial_status`, `has_subscription`, `can_create_booking`, `can_create_tournament`, `trial_ends_at`
* `public._require_commercial_access(p_club_id)` — guard interno (mismo patrón que `_require_club_not_archived`), sin `GRANT` a ningún rol de cliente, levanta `ERRCODE = 'P0008'` cuando corresponde bloquear
* Regla: sin `club_subscriptions` (club legacy) → permitido; `trialing` → permitido; `active` → permitido; `suspended` → **bloqueado**; `past_due`/`cancelled` → permitidos temporalmente, sin decisión de producto explícita todavía (ningún camino de código produce estos dos estados hoy, así que no tiene efecto observable — queda documentado para cuando alguna transición automática empiece a producirlos)

## Enforcement server-side

`_require_commercial_access` se llama dentro de las 3 únicas funciones que pueden crear el recurso real (verificado: no existe ningún INSERT directo a `reservations`/`tournaments` en ningún lugar del repo):

* `create_reservation_player` (PLAYER)
* `create_reservation_admin` (OWNER/ADMIN)
* `create_tournament` (OWNER/ADMIN)

Insertado inmediatamente después del `_require_club_not_archived` que las tres ya tenían — mismo punto, mismo patrón. Manipular el frontend, llamar la RPC directo o reconstruir la request cae exactamente en el mismo check; no hay superficie de bypass. Ningún bypass especial para SUPERADMIN — el acceso elevado ("Entrar al club") y la fila OWNER temporal de Entrega de Club quedan sujetos a la misma regla que cualquier OWNER real.

## UX por rol

* **PLAYER** — mensaje neutral ("Reservas temporalmente no disponibles"), nunca menciona pago/deuda/suscripción/Wompi
* **OWNER** — mensaje con contexto comercial explícito ("La suscripción del club no está activa...")
* **ADMIN** — mensaje que remite al propietario, sin CTA de pago

Los tres mensajes viven en un solo lugar (`shared/commercial/access.ts`), reutilizado por WEB (reservas PLAYER/OWNER/ADMIN, torneos OWNER/ADMIN) y Mobile (reservas PLAYER) — nunca una copia por plataforma. Cubierto por test automatizado (`scripts/test-commercial-entitlement.ts`, `npm run test:commercial-entitlement`) que además verifica que ni el mensaje de PLAYER ni el de ADMIN contengan ninguna palabra relacionada con pago.

## Banner global OWNER

Banner persistente ("Tu suscripción no está activa" / "Algunas funciones están temporalmente limitadas. Reactiva tu suscripción para volver a crear reservas y torneos.") agregado una sola vez en `src/app/(app)/[club]/layout.tsx` — el shell que envuelve toda página bajo `/[club]/...`, así que aparece exactamente una vez por pantalla sin haberlo agregado pantalla por pantalla. Visible solo cuando `role === "OWNER"` y `commercial_status === 'suspended'` — nunca en `trialing`/`active`, nunca en un club legacy sin suscripción, nunca para ADMIN/PLAYER, sin ningún comportamiento nuevo para SUPERADMIN. No bloquea navegación ni oculta contenido — mismo criterio del banner de club archivado ya existente en ese mismo layout, que este deliberadamente imita en vez de introducir un patrón nuevo.

**Pendiente, explícito y con seguimiento — no olvidar:** el banner todavía no tiene ningún CTA porque Wompi/reactivación no existen. Comercial v2 / Fase 3 debe agregar a este mismo banner el CTA funcional **"Reactivar suscripción"** en cuanto exista un destino real de pago — nunca antes, y nunca un enlace muerto mientras tanto.

## Mobile

Solo se modificó el flujo PLAYER de creación de reservas (`mobile/src/lib/reservationMutations.ts`). OWNER/ADMIN siguen sin ninguna superficie operativa nativa (redirigidos a WEB, ver App Móvil) y Mobile no crea torneos — ningún cambio de entitlement fue necesario ni se hizo en esos dos casos.

## Qué NO afecta la suspensión

Reservas existentes, históricas o futuras ya creadas antes de la suspensión; torneos existentes; rankings; resultados; cualquier contenido de consulta. La suspensión bloquea únicamente la **creación** de una reserva o un torneo nuevos.

## Migración aplicada en producción

* `20261115000006_commercial_entitlement.sql` — `_require_commercial_access`, `get_club_commercial_access`, y el `CREATE OR REPLACE` de las 3 funciones de creación con la línea de entitlement agregada

## QA real en producción — `Club Demo Comercial`

**`trialing`:** PLAYER puede crear reserva; OWNER/ADMIN pueden crear reserva y torneo; ningún mensaje comercial aparece; todo el contenido existente sigue funcionando.

**`suspended` — PLAYER:** entra al club, ve reservas existentes, ranking y torneos existentes con normalidad; nueva reserva bloqueada con el mensaje neutral correcto.

**`suspended` — OWNER:** navega y consulta contenido sin restricción; una reserva y un torneo creados antes de la suspensión siguen existiendo; nueva reserva y nuevo torneo bloqueados con el mensaje comercial correcto; banner global visible en Dashboard y el resto del shell; banner desaparece al volver a `trialing`.

**`suspended` — ADMIN:** ve reservas y torneos existentes; nueva reserva y nuevo torneo bloqueados; mensaje de contactar al propietario, sin CTA de pago; sin banner global.

**Legacy (club sin `club_subscriptions`):** puede crear nueva reserva y nuevo torneo con normalidad; ningún mensaje comercial aparece; no se inicializa ninguna suscripción/trial automáticamente.

**Restauración:** `Club Demo Comercial` fue regresado a `trialing` al terminar el QA — volvió a permitir creación de reserva y torneo, y el banner OWNER desapareció.

## QA técnico

`npx tsc --noEmit` sin errores; `npm run build` limpio; ESLint focalizado sin errores nuevos; lint global conserva el baseline histórico (73 errores/43 warnings); `npm run test:commercial-entitlement` → 17 passed, 0 failed; Mobile conserva sus errores TypeScript preexistentes en archivos no tocados (`reservationAdmin.ts`, `reservationExtraTime.ts`), sin errores nuevos.

## Pendiente / próxima fase (histórico — ya completado, ver Fase 3 más abajo)

Siguiente checkpoint en su momento — **Comercial v2 / Fase 3 (Pagos / Wompi)**: Wompi, checkout, `payments`/`payment_events`, webhooks, Payment Sources/tokenización, cobro automático, renovación, grace period, transiciones automáticas de estado comercial, billing UI, facturación, emails comerciales, y el CTA funcional **"Reactivar suscripción"** en el banner OWNER ya existente. Implementado desde entonces — ver "Comercial v2 — Fase 3 (Pagos Wompi + Ciclo Automático)" a continuación.

---

# Comercial v2 — Fase 3 (Pagos Wompi + Ciclo Automático)

Estado: ✅ DONE — implementada y migrada en producción antes de esta sesión de documentación (commits `468e4c1` "feat: add commercial subscriptions and Wompi payments", `12273f9` "feat: complete commercial lifecycle and reservation gating", `ef2ffd8` "Harden Wompi transaction verification"). Esta entrada se agrega retroactivamente — no se registró en su momento — reconstruida a partir del SQL real (`20261115000007_commercial_payments.sql`, `20261115000008_wompi_transaction_identity_hardening.sql`) y del código WEB existente, nunca inventada. Ver CLAUDE.md → Commercial Subscription Principles para las reglas permanentes.

## Pagos manuales Wompi

* `payments` — un intento de pago por fila (`provider='wompi'`, `provider_reference` propia y única generada antes de enviar al checkout, `provider_transaction_id`/`status`/`payment_method`/`raw_response` llenados por el webhook), snapshot propio de `base_price`/`discount_amount`/`amount`/`currency` al momento del checkout — nunca relee `commercial_settings` después de crear el pago.
* `payment_events` — auditoría + idempotencia del webhook, deduplicado por `(provider, provider_transaction_id, status)`.
* `create_pending_payment(p_club_id)` — solo el OWNER real (`club_members.role = 'OWNER'`, nunca `effective_club_role`: mover dinero real nunca pasa por el acceso elevado de SUPERADMIN). A lo sumo un pago `pending` por suscripción, garantizado por índice único parcial — un doble click/doble pestaña reutiliza el mismo intento en vez de crear uno segundo.
* `process_wompi_transaction_event(...)` — `service_role` únicamente, invocada solo desde el webhook tras validar el checksum de Wompi. En `APPROVED` abre un nuevo `subscription_periods` y pasa la suscripción a `active`, preservando siempre los días de trial/período ya pagados que quedaran por consumir (un pago anticipado nunca "pierde" días).

## Ciclo comercial automático

`run_commercial_lifecycle_transitions()` (`service_role`, disparada por `/api/cron/commercial-lifecycle` protegida con `CRON_SECRET`, idempotente por construcción) es lo único que transiciona un estado comercial automáticamente:

* `active` con `current_period_end` vencido → `past_due`
* `trialing` con `trial_ends_at` + 14 días de gracia vencidos → `suspended`
* `past_due` con `current_period_end` + 30 días de gracia vencidos → `suspended`

Ambas ventanas de gracia (14 días post-trial, 30 días post-past_due) son funciones puras de fechas que ya existen (`trial_ends_at`, `current_period_end`) — nunca columnas nuevas — calculadas igual en SQL y en `deriveCommercialPhase` (`shared/commercial/lifecycle.ts`), nunca una segunda copia que pueda desincronizarse.

## Hardening de verificación (`20261115000008`)

Verificación servidor-a-servidor de la transacción Wompi (`src/lib/wompi/verifyTransaction.ts`) antes de confiar en cualquier evento del webhook — cerró un vector real de spoofing donde una `reference` alterada sobre una transacción ya en estado final podía colarse. Cubierto por `scripts/test-commercial-payments.ts` (126 casos, incluida la reproducción exacta del exploit original y su bloqueo tras el fix).

## UI OWNER

`/[club]/subscription` — estado comercial, precio, fecha límite de gracia cuando aplica, y CTA funcional ("Pagar suscripción" / "Reactivar suscripción" cuando `suspended`) que dispara el checkout real; `/[club]/subscription/payment-result` — pantalla de retorno de Wompi, nunca confía en los query params para cambiar estado, solo los usa para consultar qué ya procesó el webhook (`get_payment_status`). El banner global OWNER (`[club]/layout.tsx`) ya no tiene el CTA pendiente que Fase 2 había dejado documentado como hueco — ese hueco quedó cerrado con esta fase.

## QA técnico verificado en esta sesión de documentación

`npm run test:commercial-payments` → 126 passed, 0 failed (incluye checksum de eventos, firma de integridad del checkout, derivación de fase comercial, y la regresión completa del hardening de Fase 5.1.1). No se tiene evidencia de primera mano del QA manual en producción realizado cuando esta fase se implementó — no se documenta ninguna cifra o escenario que no haya sido verificado directamente.

---

# Comercial v2 — Fase 6 (Superadmin Comercial)

Estado: ✅ DONE — implementada y corregida en esta sesión; todavía sin commit/push al cierre de esta actualización (`git status --short` al final de esta sección). Ver CLAUDE.md → Commercial Subscription Principles para las reglas permanentes.

Objetivo: dar a Platform Superadmin visibilidad operativa sobre suscripciones/períodos/pagos de cada club, y permitirle administrar `commercial_settings` sin Supabase SQL Editor — pura observabilidad + configuración global, sin ningún bypass del lifecycle ya construido en Fase 1-3.

## Migración

`supabase/migrations/20261115000009_platform_subscriptions_settings.sql` (aplicada manualmente en Supabase SQL Editor, luego parcheada in-place dos veces más durante QA — ver "Bugs corregidos" abajo — antes del commit; el archivo en disco es la versión final ya consistente con lo desplegado):

* `get_platform_clubs_overview()` extendida (mismas 10 columnas originales, mismo orden, columnas comerciales agregadas al final) — una sola consulta reutilizada tanto por `/platform/clubs` como por `/platform/subscriptions`, en vez de una segunda vista casi idéntica.
* `get_platform_subscription_periods(p_club_id)` / `get_platform_subscription_payments(p_club_id)` — historial angosto para el detalle de un club, nunca exponen `raw_response`/`payload`.
* `get_platform_commercial_settings()` / `update_platform_commercial_settings(...)` — lectura/escritura de la fila singleton de `commercial_settings`.

## `/platform/subscriptions` (nueva)

Listado global de todos los clubes con su estado comercial efectivo — legacy incluidos —, búsqueda por club/slug/owner/payer, filtro por fase (`Legacy`/`Trial`/`Gracia trial`/`Activo`/`Past due`/`Suspendido`), métricas simples (conteos, nunca MRR/forecasting), cada fila enlaza al detalle ya existente en `/platform/clubs/[clubId]`. El badge de estado se calcula con `shared/commercial/platformBadge.ts` (nuevo), que envuelve `deriveCommercialPhase` — nunca una segunda regla de estado para esta audiencia.

## `/platform/settings` (nueva)

Lee/edita `commercial_settings`: precio base, promo activa/precio, días de trial; moneda readonly (solo COP). Validación cliente + servidor + RPC (triple capa). Reglas de gracia (14/30 días) se muestran como texto informativo fijo, sin nueva columna editable. Ningún cambio de configuración toca `subscription_periods`/`payments` históricos — estructuralmente imposible, la RPC de escritura solo hace `UPDATE` sobre `commercial_settings`.

## `/platform/clubs`

Columna nueva "Suscripción" (badge de estado comercial) agregada junto a la columna "Estado" (operativo) ya existente — ambos conceptos quedan visualmente separados, nunca fusionados.

## `/platform/clubs/[clubId]`

Secciones nuevas "Períodos" y "Pagos" (historial completo, solo lectura) agregadas a la sección "Estado comercial" ya existente de Fase 1; el badge de estado ahí también pasó a usar `deriveCommercialPhase` (antes distinguía únicamente por `status` crudo, sin separar `trial` de `trial_grace`).

## Seguridad

Cada RPC nueva valida `is_platform_admin` internamente — inalcanzable por llamada directa. `payments`/`subscription_periods`/`commercial_settings` siguen sin ninguna policy RLS (patrón cerrado ya existente). Ningún dato financiero sensible (`raw_response`, `payload`, checksums) se expone en ninguna pantalla.

## QA técnico

`npx tsc --noEmit` limpio; `npm run build` exitoso con ambas rutas nuevas registradas; `npm run test:commercial-entitlement` → 30/30; `npm run test:commercial-payments` → 126/126; ESLint global idéntico al baseline (73 errores/44 warnings, verificado con `git stash` antes/después) en cada uno de los checkpoints de esta sesión.

## Bugs reales encontrados y corregidos durante el cierre de QA de esta fase

Ninguno de los tres es específico de Fase 6 — son bugs preexistentes en flujos de navegación/config que el QA de esta fase hizo evidentes. Ver el detalle completo en la sección siguiente ("Corrección — Navegación Multi-Club y Club Desactivado") y en "Corrección — `update_platform_commercial_settings` (UPDATE sin WHERE)" más abajo:

1. Un OWNER multi-club con `last_club_id` apuntando a un club operativamente desactivado quedaba atrapado ahí en vez de entrar a sus otros clubes activos.
2. El mismo problema, vía selección manual desde el modal "Cambiar de club".
3. `/clubs` ("Mis clubes") no distinguía visualmente un club desactivado de uno navegable.
4. Guardar `/platform/settings` fallaba siempre con un mensaje genérico — causa real: `UPDATE public.commercial_settings` sin `WHERE`, rechazado por este proyecto de Supabase con `SQLSTATE 21000`.

---

# Corrección — Navegación Multi-Club y Club Desactivado

Estado: ✅ DONE — corregido en esta sesión, sin commit/push todavía. Ver CLAUDE.md → Club Archival Principles para la regla permanente.

Bug real encontrado durante QA de Fase 6 (no relacionado con lo comercial en sí): la desactivación de un club es POR CLUB, nunca por usuario, pero tres caminos de resolución de club dejaban a un OWNER/ADMIN/PLAYER multi-club atrapado en un club operativamente desactivado (`clubs.is_active = false`) aunque tuviera otros clubes activos, porque solo filtraban `archived_at` y nunca `is_active`.

## Archivos corregidos

* `src/lib/utils/navigation.ts` — `resolveClubEntryPath` (login, callback OAuth, `/notifications`, `LeaveClubButton`) y `resolveActiveMembership` (`/profile`, sidebar) ahora excluyen `is_active = false` del conjunto elegible, igual que ya excluían `archived_at`. Un usuario cuyo `last_club_id` apunta a un club desactivado, con otro club activo disponible, ahora entra normalmente al activo (o al selector `/clubs` si hay ambigüedad) en vez de quedar atrapado.
* `src/lib/actions/clubSwitcher.ts` — `getOwnerClubs` (detrás del modal "Cambiar de club" del OWNER) excluye igual los clubes desactivados de la lista de destinos seleccionables.
* `src/app/clubs/page.tsx` — "Mis clubes" sigue mostrando un club desactivado a propósito (pantalla informativa), pero ya no lo presenta como navegable: badge "Desactivado", tarjeta atenuada (`opacity-60`), sin `Entrar →`, renderizada como `<div>` no como `<Link>` — cero forma de navegar hacia él desde esa tarjeta.

## Por qué `ClubDeactivatedScreen` hacía esto grave

`[club]/layout.tsx` reemplaza toda la pantalla por `ClubDeactivatedScreen` sin AppNav ni selector de clubes cuando `!club.is_active` — solo un enlace "Volver al inicio". Sin el fix, cualquiera de los tres caminos de arriba podía dejar a un usuario multi-club sin salida visible hacia sus clubes que sí funcionaban.

## QA (verificado por lectura de código, sin entorno de reproducción con múltiples usuarios reales disponible)

OWNER multi-club + último activo → sin cambio. Último inactivo + otro activo → ya no atrapa, entra al activo o al selector. `last_club_id` inválido → recuperación sin cambio. Solo club(es) inactivo(s) → cae a `/clubs` (selector), una de las dos experiencias explícitamente aceptadas para este caso. PLAYER sin club → sin regresión. Club activo con suscripción `suspended` → sigue siendo navegable normalmente (el fix depende únicamente de `clubs.is_active`, nunca de `club_subscriptions`). SUPERADMIN → sin regresión (no usa estas funciones). `tsc`/ESLint focalizado limpios en cada archivo tocado; `test:commercial-entitlement`/`test:commercial-payments` conservan su baseline.

---

# Corrección — `update_platform_commercial_settings` (UPDATE sin WHERE)

Estado: ✅ DONE — corregido en esta sesión y parcheado manualmente en la base ya desplegada. Ver CLAUDE.md → Shared View & Data Patterns (nueva regla permanente sobre `UPDATE`/`DELETE` sin `WHERE` en este proyecto).

Guardar `/platform/settings` fallaba siempre con "No se pudo guardar la configuración. Intenta de nuevo." Logging temporal agregado y luego retirado en `src/app/platform/settings/actions.ts` reveló el error real: `code: '21000'`, `message: 'UPDATE requires a WHERE clause'`. Este proyecto de Supabase rechaza cualquier `UPDATE`/`DELETE` sin `WHERE`, incluso sobre una tabla singleton (`commercial_settings`, garantizada por `commercial_settings_singleton_idx` — un índice único sobre la expresión constante `(true)`, nunca sobre un id fijo/conocido).

Fix aplicado en `supabase/migrations/20261115000009_platform_subscriptions_settings.sql` (y parcheado manualmente en producción con el mismo `CREATE OR REPLACE FUNCTION`): la función ahora resuelve `v_settings_id` con `SELECT id INTO v_settings_id FROM commercial_settings LIMIT 1` (falla con `ERRCODE = '55000'` si no existe), y el `UPDATE` lleva `WHERE id = v_settings_id` explícito — nunca `WHERE true`, nunca un UUID hardcodeado. Autorización, las 4 validaciones existentes, la firma de la RPC y los `GRANT`/`REVOKE` quedaron sin cambios. Prueba real en navegador confirmada por el usuario: guardar `99990 → 99991` persiste tras recargar; restaurado a `99990` sin dejar el valor de prueba en producción.
...
`RootNavigator.tsx` resuelve estructuralmente, antes de montar ningún tab, a cuál de tres árboles entra la sesión según `useClub()` (`club`/`role` del CLUB ACTIVO, nunca de otra membresía que la cuenta tenga en otro club):

* **Sin club activo** → `NoClubStack` (Explorar clubes), sin cambios
* **OWNER/ADMIN** (desde esta ronda) → `OwnerAdminStack`, una única pantalla (`OwnerAdminWebScreen`) — mobile v1 no tiene ningún dashboard/tab operativo nativo para este rol; ver "Alcance v1 de OWNER/ADMIN" más abajo. `OwnerAdminTabs` (el navegador de tabs que antes se montaba para este rol: Inicio | Jugadores | Reservas | Torneos | Ranking | Club) queda en el código sin usar — no se borró, pero ya no es alcanzable
* **PLAYER** → `MainStack` (`AppTabs.tsx` → `PlayerTabs`, sin cambios de navegación en esta ronda): Dashboard | *(nombre real del club, ícono pelota de tenis)* | Reservaciones | Ranking | Torneos — mismo orden que `PLAYER_TAB_BAR_LABELS` en `AppNav.tsx`

**Incidente real corregido antes de esta ronda, no solo cosmético**: el `RankingTab`/`ClubTab`/`PlayersTab` de PLAYER llegaron a ser literalmente los mismos de OWNER/ADMIN, sin ningún guard de rol — un PLAYER podía abrir la pantalla "Jugadores" (roster administrativo completo, incluido el WhatsApp de cualquier otro miembro), una violación real de CLAUDE.md → Player Contact Principles/Privacy Principles. Ver CLAUDE.md → Shared View & Data Patterns para la regla permanente.

### Alcance v1 de OWNER/ADMIN (nuevo)

Decisión de producto para el cierre de MVP: la app nativa no construye ninguna superficie operativa para OWNER/ADMIN. `OwnerAdminWebScreen` (`mobile/src/screens/OwnerAdminWebScreen.tsx`) es la única pantalla que ve este rol — título "Administración del club", texto explicativo, botón "Ir a MiPadel.club" (`Linking.openURL("https://mipadel.club/auth/login")`) y "Cerrar sesión" (misma confirmación que ya usaba `AppHeader.tsx`). Deliberadamente sin "Cambiar de club" (se probó y se quitó): un OWNER/ADMIN con más de un club sigue cambiando solo desde WEB. La web sigue resolviendo login/rol/club exactamente como ya lo hacía — este botón es solo una puerta hacia ella, sin auto-login, sin transferencia de sesión, sin deep link. `OwnerAdminStack` (`RootNavigator.tsx`) es la única ruta que monta esta pantalla; PLAYER nunca pasa por este código.

### Login rediseñado (nuevo)

Mismo formulario/lógica de `signInWithPassword` de siempre (sin tocar). Encabezado ahora con el isotipo real (`mobile/assets/branding/logo-icon.png`, copia exacta del PNG que ya usa `BrandLogo.tsx`/`Footer.tsx` en WEB — copiado dentro de `mobile/` porque `metro.config.js` solo resuelve `projectRoot` + `shared/`) + wordmark "Mi"/"Padel"/"Club" compuesto por texto en los colores de marca (mismo patrón que `BrandLogo.tsx`, nunca un logo recreado desde cero ni un PNG de wordmark completo, que no está en uso real en ningún lado de WEB), titular "Todo tu pádel en un solo lugar" + descripción corta, "Conocer más →" (`https://mipadel.club/`). Fondo real de cancha/pelotas/palas (`mobile/assets/background.png`, `ImageBackground` cubriendo toda la pantalla incluido detrás del status bar, overlay muy sutil `rgba(0,10,15,0.32)` solo para blindar contraste). Tarjeta de login sin cambios de diseño; debajo del botón "Entrar" se agregó "¿Aún no tienes cuenta? Crear cuenta" → `https://mipadel.club/` (mismo destino que "Conocer más" — `/auth/register` no existe todavía porque el registro real requiere elegir jugador/propietario, selección que hoy solo resuelve el landing).

## Bug crítico corregido — resolución de club/identidad tras login

Reportado como bloqueante para el release: un PLAYER con una membership activa real, al hacer login, caía en la pantalla de "sin club" (`ChangeClubScreen`/Explorar clubes) en vez de entrar a su Dashboard — tocar el club correcto ahí no hacía nada, y tocar la X cerraba la sesión (síntoma secundario, no la causa). Por separado, cuando sí lograba entrar, el Dashboard mostraba su email crudo como nombre y el avatar caía al fallback de inicial, en vez de `profiles.full_name`/`avatar_url` reales.

Causa raíz única, confirmada con datos reales de un usuario de prueba (dos membresías activas válidas, `last_club_id` apuntando correctamente a una de ellas, `full_name`/`avatar_url` presentes en `profiles` — el problema nunca fue de datos): `resolveActiveMembership`/`getIdentity` (`mobile/src/lib/activeMembership.ts`/`userIdentity.ts`) desestructuraban solo `{ data }` de sus consultas a `club_members`/`profiles`, descartando el `error` — un fallo real y transitorio de esa única consulta (confirmado: `anon` sin sesión recibe `42501 permission denied` sobre esas mismas tablas, la clase de error que ocurriría en una ventana de carrera justo después de `signIn()`) se leía exactamente igual que "el usuario no tiene nada", sin distinguir "no pude saberlo" de "confirmé que está vacío". Fix: ambas funciones ahora capturan el `error`, lo propagan (`throw`) en vez de tragarlo, y reintentan una vez de inmediato — un fallo transitorio se auto-resuelve dentro de la misma carga en vez de dejar la sesión varada. Nueva regla permanente en CLAUDE.md → Shared View & Data Patterns.

## Qué está portado, por módulo

* **Dashboard PLAYER** (`HomeScreen` → `PlayerDashboardScreen`) — paridad completa con `/[club]/dashboard` (encabezado deportivo, próxima actividad, evolución, resumen, mis torneos, logros, actividad reciente). "Mis torneos" (`MyTournamentsSection.tsx`) pasó de una lista vertical de `TournamentCard` grandes a un carrusel horizontal compacto (`CompactCarouselCard`, mismo patrón que Página del club — imagen+título, ~44% de ancho), manteniendo exactamente los mismos datos/filtro/navegación a `TournamentDetail`
...
* **Torneos** — detalle, inscripciones, clasificación en vivo, podio con confetti sin cambios. El listado (`TournamentsScreen.tsx`) ganó dos ajustes en esta ronda: (1) el tab inferior "Torneos" ya siempre abre este listado — antes podía reabrir directo el `TournamentDetail` de un torneo visitado antes, porque el stack anidado de ese tab se preserva por defecto al cambiar de tab (fix: listener `tabPress` en `AppTabs.tsx` que hace `navigate()` de vuelta a `TournamentsList`, sin tocar push/deep links ni ningún otro caller que abre `TournamentDetail` directo); (2) la presentación de resultados pasó de una lista vertical de `TournamentCard` grandes a un grid de 2 columnas con `CompactCarouselCard` (mismas queries/filtros/estados, ancho de card calculado desde `useWindowDimensions()`, nunca hardcodeado). **Gap conocido, sin corregir todavía** (preexistente, no introducido en esta ronda): esta pantalla sigue siendo el mismo listado ADMIN-only de siempre — copy "Administra inscripciones, duplas y clasificación de tus torneos.", botón "Crear torneo" y los 7 tabs de `ADMIN_TAB_ORDER` (incluido "Borradores") se muestran a cualquier rol que entre, a diferencia del resto de la navegación mobile, que ya está correctamente gateada por rol
...
* **Explorar clubes / página pública** (`ChangeClubScreen` + `PublicClubScreen`, nueva) — paridad funcional con `/[club]` + `RequestAccessButton` de WEB: buscador/filtros/mapa de `ChangeClubScreen` intactos, pero tocar una fila que no es una membresía propia ya no ejecuta join/request directo — navega a `PublicClubScreen` (portada, logo, nombre, ubicación, badge público/privado, descripción, métricas, galería si hay fuente reutilizable, redes sociales) con el CTA correcto ("Solicitar ingreso"/"Unirme"/"Solicitud pendiente" deshabilitado/"Entrar al club"), revalidado contra Supabase (nunca el param de navegación) en cada montaje y en cada foco. `WhatsAppRequiredModal` extraído a un componente propio, usado solo aquí — `ChangeClubScreen` ya no lo conoce
* **Push notifications (Fase 1)** — ver sección propia más abajo
...
Solo registro + tap-to-open — nunca un sistema paralelo al de notificaciones ya existente (bell/dropdown/`/notifications`), ver CLAUDE.md → Notifications & Live-Update Principles.

* **Registro**: `registerPushToken` (`mobile/src/lib/pushNotifications.ts`) corre una vez por sesión desde `RootNavigator.tsx` apenas hay `session` — pide permiso, obtiene el Expo push token, y lo persiste vía la RPC `register_push_token` (`SECURITY DEFINER`, toma `profile_id` siempre de `auth.uid()`, nunca de un parámetro — resuelve la reasignación de un mismo dispositivo a otro usuario, que un `upsert` directo del cliente no podía por RLS)
* **Envío**: Edge Function `supabase/functions/send-push/index.ts` (Deno) + trigger de base de datos que la invoca en cada `INSERT` sobre `notifications` — el envío real sigue siendo "mejor esfuerzo", nunca bloquea ni duplica la notificación in-app ya creada
* **Recepción/tap-to-open** (`PushNotificationNavigator.tsx`, montado una sola vez dentro de `ClubProvider`): cubre listener en vivo (`addNotificationResponseReceivedListener`), cold-start y el fallback de `AppState→active` — los tres funnelean a la misma `processResponse`/`openFromNotificationId`, que revalida todo contra Supabase (nunca confía en el payload de la push) y reutiliza exactamente `resolveNotificationTarget`/`buildTabNav` ya usados por `NotificationsScreen.tsx`
* **Bug real corregido — respuesta vieja reapareciendo**: `Notifications.getLastNotificationResponseAsync()` es un getter de un único slot nativo persistente, no un stream — sin `Notifications.clearLastNotificationResponse()` tras consumir cada respuesta, una aprobación ya procesada volvía a dispararse en cada `AppState→active`, eclipsando cualquier push nueva (p. ej. `player_deactivated`) y sobreviviendo incluso a un reload de Metro (el dedup en memoria por `notification_id` no alcanza por sí solo). Ver CLAUDE.md → Notifications & Live-Update Principles
* **Bug real corregido — loop de splash**: revalidar `ClubContext` desde una push (o desde `ChangeClubScreen` al recuperar foco) reutilizaba la misma bandera `loading` que bloquea el arranque — cada revalidación desmontaba `MainStack`/`NoClubStack` de vuelta al splash, y para un usuario sin club activo eso remontaba la misma pantalla que disparó la revalidación, en loop infinito. Fix: `reloading` separado, bootstrap-only sigue siendo `loading`
* **Bug real corregido — navegación post-aprobación perdida**: tras aprobar el primer acceso de un jugador y tocar la push, `navigateIntoApp()` corría inmediatamente después del `await reloadClub()`, compitiendo contra el remonte estructural `NoClubStack → MainStack` y perdiendo la carrera — el usuario quedaba viendo `PublicClubScreen` con "Solicitud pendiente" pese a que el backend ya lo había aprobado. Fix: `awaitingClubEntry`, el mismo patrón de espera determinista que ya usaba `pendingRootNav` (gated en un nuevo `reloading`, no en `loading`)
* **Validado end-to-end en dispositivo físico real** (no solo `tsc`/build) con logs de diagnóstico temporales (ya removidos): primer acceso → aprobación → push → entra al club; background/foreground sin tocar push → no reprocesa; desactivación → push → Explorar clubes; reingreso tras desactivación/rechazo → aprobación → push → entra de nuevo

## Pendiente / gaps conocidos

* `TournamentsScreen.tsx` sin gate de rol real (ver arriba) — ya no aplica a OWNER/ADMIN (ver "Alcance v1 de OWNER/ADMIN": ese rol nunca llega a esta pantalla en mobile), pero sigue siendo relevante si algún día un rol distinto de PLAYER vuelve a alcanzarla
* Ranking/Club (`PlaceholderScreen`) quedan sin uso real desde el alcance v1 de OWNER/ADMIN — ya no son alcanzables por ningún rol en mobile; no se removieron del código en esta ronda
...
* Validación visual (diseño/layout) de `PublicClubScreen` en dispositivo real pendiente — su flujo funcional (CTA, revalidación, navegación tras push) sí fue validado en dispositivo físico real (ver Push Notifications arriba), pero no hubo una revisión visual dedicada de su layout
* **Validación visual pendiente, esta ronda**: Login rediseñado (isotipo/wordmark/fondo/overlay/"Conocer más"), `OwnerAdminWebScreen`, el grid de 2 columnas de `TournamentsScreen.tsx` y el carrusel de "Mis torneos" del Dashboard se verificaron con `tsc`/lectura de código, nunca en un dispositivo o simulador real — falta una revisión visual dedicada de los cuatro antes de considerarlos definitivos

---

# Corrección de Build — Tipos Generados de Supabase vs. SQL Real

`npm run build`/`npx tsc --noEmit` en WEB quedaron bloqueados por una cadena de errores `Type 'string | null' is not assignable to type 'string'` — ninguno era un bug de negocio: en cada caso, el SQL real de la RPC acepta y maneja `NULL` explícitamente (`COALESCE`/`NULLIF`, comparaciones `IS NOT NULL`, o como señal literal de "crear nuevo" en un upsert), pero el generador de tipos de Supabase marca como obligatorio y no-nulo cualquier parámetro sin `DEFAULT` en la firma SQL, y cualquier columna de un `RETURNS TABLE` sin importar si viene de un `LEFT JOIN`. Cada mismatch se investigó contra el SQL vigente (nunca se asumió, nunca se aplicó `?? undefined`/`as string` a ciegas) antes de corregir. Nueva regla permanente en CLAUDE.md → Shared View & Data Patterns.

Fixes aplicados, todos como un cast local documentado en el call site (nunca tocando `shared/types/database.ts` ni el SQL):

* `shared/tournaments/actions.ts` — `createTournament()`/`create_tournament` (7 campos, todos con `DEFAULT NULL` en SQL → `?? undefined`, único caso correcto para esa forma); `updateTournament()`/`update_tournament` y `updateTournamentCoverImage()`/`update_tournament_cover_image` (sin `DEFAULT` en SQL → `as string`, preservando el `null` real en runtime)
* `src/app/(app)/[club]/admin/news/actions.ts` — `create_club_news`, `p_tournament_id` (`NULL` = noticia general, sin torneo asociado)
* `src/app/(app)/[club]/admin/reservations/actions.ts` — `create_reservation_admin`/`update_reservation_admin` (`p_title`/`p_notes`) y `add_reservation_extra_time` (`p_note`)
* `src/app/(app)/[club]/admin/settings/actions.ts` — `upsert_pricing_rule_with_prices`, `p_rule_id` (`NULL` = crear regla nueva, la señal real del `IF p_rule_id IS NULL THEN INSERT ELSE UPDATE`) y `p_court_id` (`NULL` = regla a nivel club, sin cancha específica)
* `src/app/claim-club/[token]/actions.ts` — `claim_club`, `p_ip` (único caso con `DEFAULT NULL` real en SQL → `?? undefined`, correcto)
* `src/app/platform/clubs/[clubId]/page.tsx` — retorno de `get_club_claim_status` no conservaba el union literal de `status` (`"pending" | "claimed" | "revoked"`, garantizado por un `CHECK` de la tabla) ni la nulabilidad real de `claimed_at`/`claimed_by_name`/`claimed_by_email`

Además, `tsconfig.json` excluye ahora `supabase/functions/` (Edge Functions, runtime Deno, especificadores `npm:paquete@version` que Next.js no puede resolver) — el `include` amplio (`**/*.ts`) las barría sin querer al mismo programa de TypeScript que `next build` usa para el resto del proyecto, igual que ya le pasaba a `mobile/` antes de excluirlo.
...
* Ladder
* Comunidad
* Cobro recurrente/tokenizado (Payment Sources), refunds, múltiples planes/tarifas, facturación electrónica, emails comerciales
...
El Funnel Comercial v1 (captación de leads → gestión SUPERADMIN → conversión a Entrega de Club), Comercial v2 / Fase 1 (capa comercial base + trial automático de 30 días al reclamar el club), Comercial v2 / Fase 2 (entitlement — bloqueo de creación de reservas/torneos nuevos cuando el club está `suspended`, más el banner global OWNER), Comercial v2 / Fase 3 (pagos manuales Wompi, ciclo comercial automático con transiciones y gracia, CTA de pago/reactivación funcional) y Comercial v2 / Fase 6 (observabilidad y configuración global para Platform Superadmin) tampoco son funcionalidades futuras — los cinco están implementados (ver Funnel Comercial (v1), Comercial v2 — Fase 1, Fase 2, Fase 3 y Fase 6). Lo que sigue genuinamente sin implementarse es el ítem de cobro recurrente/tokenizado listado arriba — el estado operativo `Activo` de un club nunca debe confundirse con una suscripción o trial activo.
...
Pendiente inmediato, no funcional: todo el trabajo de Comercial v2 / Fase 6 y sus tres correcciones de navegación multi-club/club desactivado (ver esas secciones) está implementado y verificado técnicamente, pero **sin commit ni push todavía** — el árbol de trabajo tiene estos cambios sin confirmar al momento de esta actualización.
