// Texto del mensaje de "WhatsApp" — portado de src/lib/reservationShare.ts
// (app web). Una sola función para que los puntos que ya comparten/copian
// el enlace de una reserva (ReservationTicketPanel, PlayerActivity,
// ReservationShareView en WEB; RequestModal/ReservationDetailScreen en
// mobile) nunca tengan versiones ligeramente distintas del mismo mensaje
// — un mensaje compartido desde la app nativa es indistinguible de uno
// compartido desde la web.
//
// SITE_URL NO vive aquí: en WEB, cada caller arma su propio `url` con
// `window.location.origin` (el navegador siempre conoce su propio
// origen); mobile no tiene ese concepto (es una app nativa) y por eso
// mantiene su propia constante local (ver mobile/src/lib/reservationShare.ts)
// — no es una regla de negocio duplicada, es infraestructura específica
// de cada plataforma para resolver el mismo parámetro `url` que esta
// función ya recibe como input puro.
