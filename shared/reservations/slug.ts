...
// cosmética. Portado de src/lib/reservationSlug.ts (app web). La reserva
// SIEMPRE se resuelve por su uuid real (reservations.id) o, para el slug
// corto, por una búsqueda exacta (club + fecha/hora + nombre del creador)
// resuelta server-side vía la RPC resolve_reservation_slug
// (20261103000001) — el texto del slug nunca es, por sí solo, la fuente
// de verdad ni concede acceso a nada: solo apunta a un uuid, y ese uuid
// pasa exactamente por el mismo control de acceso
// (get_reservation_share_detail) que ya existía.
//
// Tres formatos:
//   1. uuid puro                          — enlaces ya compartidos antes.
//   2. "nombre-fecha-uuid" (slug anterior) — el formato del turno pasado.
//   3. "nombre-YYYYMMDDHHmm" (slug nuevo)  — el formato que se genera
//                                            ahora, sin uuid visible.
// Los formatos 1 y 2 siempre contienen un uuid real en el texto —
// extractReservationId lo encuentra con una regex. El formato 3 nunca
// contiene un uuid — se reconoce porque termina en exactamente 12
// dígitos, y se resuelve vía resolve_reservation_slug. mobile nunca
// navega por URL entrante, así que solo usa slugifyNamePart/
// buildReservationSlug (para construir el mensaje de WhatsApp/enlace
// copiado); extractReservationId/parseShortReservationSlug son de uso
// exclusivo de WEB, pero viven aquí igual — es la misma normalización
// exacta la que ambos lados necesitan mantener sincronizada si algo
// cambia.

const UUID_RE = /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i;
// Nombre + exactamente 12 dígitos (YYYYMMDDHHmm) al final — nunca
// coincide con un slug que ya contiene un uuid (36 caracteres con
// guiones, nunca 12 dígitos puros consecutivos).
 ...
 *  una coincidencia EXACTA de nombre — sin un nombre real, ningún nombre
 *  normalizado puede matchear jamás, así que un placeholder tipo
 *  "reserva-YYYYMMDDHHmm" queda permanentemente sin resolver (bug real,
 *  confirmado). Por eso, sin nombre real disponible, el fallback incrusta
 *  el uuid real (`id`) en vez de un nombre inventado —
 *  extractReservationId lo encuentra con su regex desde cualquier parte
 *  del texto. */
