...
// Picks the single soonest booking to headline "Próxima actividad" —
// reutiliza getReservationTimeStatus (shared/reservations/editPolicy.ts),
// la misma clasificación future/ongoing/past ya usada por las políticas de
// edición de reserva, evaluada contra el "ahora" canónico de Bogotá
// (getBogotaNow) — nunca new Date()/Date.now() crudo, que reflejaría la
// zona del PROCESO (el servidor de Next.js que renderiza este Dashboard
// corre en UTC salvo que se fije TZ, algo que este repo nunca hace) en vez
// de la del club. Antes esta función solo miraba el START (`start <=
// now`), así que una reserva confirmada de HOY cuyo horario ya había
// terminado seguía ganando como "en curso" y encabezando "Próxima
// actividad" — bug real confirmado, nunca un estado nuevo de reserva:
// `status` sigue siendo exactamente el que ya existe (`confirmed` no deja
// de serlo al terminar), solo cambia qué se considera "próxima" según el
// FIN real (date + start_time + duration_minutes), nunca solo el status.
// "ongoing" se prioriza primero, igual que antes — mismo comportamiento
// canónico que editPolicy.ts ya define para el resto del producto (nunca
// una copia separada de esa regla).
export function pickNextBooking(myBookings: MyReservation[], now: BogotaNow = getBogotaNow()): MyReservation | null {
  const candidates = myBookings
    .map((reservation) => ({
      reservation,
      status: getReservationTimeStatus(reservation.date, reservation.start_time, reservation.duration_minutes, now),
    }))
  ...
  candidates.sort((a, b) => {
    ...
    return (a.reservation.date + a.reservation.start_time).localeCompare(b.reservation.date + b.reservation.start_time);
  });

  return candidates[0].reservation;
}
...
export interface PlayerActivityEvent {
  ...
  label: string;
  href: string | null;
  // Redundante con `href` para WEB (que solo navega por URL) pero
  // necesario para mobile, que navega por id de pantalla, no por ruta —
  // nunca recalculado aparte, siempre el mismo `t.id` ya resuelto en este
  // mismo loop al construir `href`. null cuando el evento no es de torneo
  // (WEB tampoco tiene un id de reserva útil que ofrecer aquí: su propio
  // href de reserva ya apunta solo al listado, nunca a una reserva
  // puntual, así que mobile replica ese mismo destino, no uno nuevo).
