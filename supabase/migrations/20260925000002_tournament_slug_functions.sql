-- ============================================================
-- Torneos — funciones actualizadas para slug (URL legible)
-- Mi Pádel Club
-- ============================================================
-- create_tournament genera y persiste el slug de forma seguro ante
-- concurrencia: intenta insertar con el slug base (nombre slugificado);
-- si el índice único (club_id, slug) rechaza el INSERT por colisión,
-- reintenta agregando la fecha (starts_at o now() si no hay starts_at)
-- en formato YYYYMMDD, y si aún colisiona, agrega un sufijo incremental
-- -2, -3, ... La unicidad real siempre la exige Postgres vía el índice
-- único — este bucle nunca "adivina" que un slug está libre a partir de
-- un SELECT previo (esa secuencia sería vulnerable a una carrera entre
-- dos solicitudes simultáneas); cada intento es un INSERT real, y solo
-- se reintenta cuando ese INSERT fue rechazado por la restricción.
--
-- Las otras 6 funciones que retornan la fila completa de tournaments
-- (update_tournament, open/close/reopen_tournament_registration,
-- cancel_tournament, start_tournament) no generan ni modifican el slug
-- — update_tournament en particular nunca lo toca, ni siquiera cuando
-- cambia el nombre, para no romper URLs ya compartidas — pero sí deben
-- incluirlo en su RETURNS TABLE para seguir reflejando la forma real de
-- la tabla (regla ya documentada en este módulo: un RETURNS TABLE
-- desincronizado de la tabla real es un bug, no una omisión inocua).
-- Todas requieren DROP FUNCTION explícito antes de CREATE: Postgres no
-- permite cambiar el tipo de retorno vía CREATE OR REPLACE.
-- ============================================================
...
BEGIN
  ...
  -- Slug: intento real de INSERT por iteración, nunca un SELECT de
  -- disponibilidad seguido de un INSERT separado — así el índice único
  -- (club_id, slug) es la única fuente de verdad, incluso bajo carrera.
...
-- ─── update_tournament ────────────────────────────────────────────────
-- Nunca escribe slug — permanece estable después de crear el torneo,
-- incluso si el nombre cambia (evita romper URLs compartidas).
