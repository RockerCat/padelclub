-- ============================================================
-- close_tournament_registration — require ≥2 confirmed pairs
-- Mi Pádel Club
-- ============================================================
-- New business rule: closing registration with 0 or 1 confirmed pair is
-- no longer allowed — the organizer must either keep registration open
-- or cancel the tournament instead (cancel_tournament, untouched). 2+
-- confirmed pairs is always enough, even below max_pairs.
--
-- Only 'confirmed' tournament_entries count — pending, withdrawn and
-- rejected never do, matching capacity.confirmed (the same aggregate
-- TournamentDetailView already reads for canStart's own ">= 1"
-- threshold, see src/lib/tournamentEntries.ts).
--
-- While touching this function, also fixed the same ambiguous "club_id"
-- column reference bug already found and fixed in create_tournament
-- (20261013000001) and open_tournament_registration (20261014000001):
-- close_tournament_registration's SUPERADMIN-fallback NOT EXISTS
-- subquery referenced public.club_members without an alias, which is
-- ambiguous against this function's own RETURNS TABLE "club_id" output
-- column (real error: 42702, "column reference \"club_id\" is
-- ambiguous... PL/pgSQL variable or a table column"). Left unfixed, the
-- new rule below would never even be reached — every call would fail
-- before that point, for every caller. Same fix as the other two: alias
-- the subquery (cm2) and qualify its columns. No other line changes.
-- ============================================================
